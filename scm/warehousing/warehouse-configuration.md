...
Les entités que vous créez pour définir votre disposition d'entrepôt sont utilisées dans les requêtes que vous paramétrez dans les modèles de travail pour piloter les ordres d'exécution dans l'entrepôt. Par conséquent, lorsque vous définissez les zones, les types d'emplacement, etc., prenez en compte la manière dont les différentes zones de l'entrepôt sont utilisées pour différents processus. En outre, tenez compte des facteurs tels que les caractéristiques physiques d'une région spécifique. Par exemple, il peut y avoir des secteurs où vous pouvez utiliser qu'un certain type de chariot élévateur. Ou, si votre société a la production et le produit fini au sein de le même établissement, vous pouvez créer un seul entrepôt dans Dynamics 365 pour les opérations sans possibilité de séparer les deux opérations en créant deux groupes de la région. Donnez à vos entités les noms descriptifs, de sorte qu'il est facile de les identifier si vous les utilisez dans les requêtes de modèle.
...
Les limites de bas des emplacements afin de garantir que le travail n'est pas créé pour demander ce stock n'est pas mis à un emplacement qui n'a pas de capacité physique à retenir le stock. Par exemple, si certaines emplacements d'un entrepôt peuvent contenir uniquement une palette par emplacement, les limites de bas de l'emplacement peuvent être activées. ** Quantité ** la valeur peut être définie ** 1 **, et ** unité ** la valeur peut être définie ** PL ** dans un regroupement de profil spécifique de l'emplacement. 
...
Pour créer rapidement les emplacements d'un entrepôt, vous pouvez utiliser ** emplacement paramétré ** l'Assistant. Dans le cadre de ce processus, vous pouvez facilement conserver le format des noms d'emplacement.
