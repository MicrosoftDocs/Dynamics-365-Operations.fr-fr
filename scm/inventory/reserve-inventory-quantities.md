...
| Réservation de ligne de vente d'articles  | Détermine si la réservation d'article est manuelle ou automatique. Si une réservation est automatiquement, le stock est réservé lors de la création de lignes de commande. Il est possible de créer des réservations au niveau du nombre d'articles pour les nomenclatures (option **Automatique**), ou pour les éléments individuels d'une nomenclature (option **Éclatement**). La valeur par défaut pour ** réservation de ventes d'articles ** peut être hérité ** les paramètres du module Ventes. ** Dans cette page, la valeur est définie dans le champ Réservation dans ** les valeurs par défaut des ventes ** ** la section ** sur ** général ** l'onglet. |
