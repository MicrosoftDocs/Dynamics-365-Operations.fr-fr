...
Dans Dynamics AX, la chaîne de valeur de l'état futur peut être modélisée en tant que version de flux de production. Tous les processus de la chaîne de valeur sont modélisés en tant qu'activités de processus. Les mouvements ou les transferts peuvent être modélisés en tant qu'activités de transfert si le statut de transfert doit être enregistré ou si une intégration dans les prélèvements de stock ou les expéditions consolidées est requise. 

La chaîne de valeur elle-même est modélisée en tant qu'unité opérationnelle dans Dynamics AX. Par conséquent, la chaîne de valeur peut être utilisée comme dimension financière.
