...
description: "Cet article décrit comment personnaliser Microsoft Dynamics 365 pour les opérations."
author: RobinARH
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Cet article décrit comment personnaliser Microsoft Dynamics 365 pour les opérations.

Plusieurs types de personnalisations dans Microsoft Dynamics 365 pour les opérations. Certaines personnalisations sont des sélections effectuées dans la liste des options dans une page de paramétrage. Plusieurs exemples de personnalisations sont implicites, par exemple, Dynamics 365 pour les opérations contient les largeurs des colonnes de grille si vous les ajuster, et l'État développé/réduisent les organisateurs. D'autres personnalisations sont explicites. Pour les personnalisations explicites, vous entrez en mode interactif de personnalisation et modifiez l'apparence de la page en gérant directement la manière dont les éléments apparaissent ou agissent sur la page. 

Toutes les personnalisations, de tout type, qu'un utilisateur fait dans Dynamics 365 pour les opérations ont pour cet utilisateur uniquement, indépendamment de la société avec laquelle l'utilisateur d'installation interagit. Les modifications qu'un utilisateur effectue sur une page n'affectent pas les autres utilisateurs dans le système.

## <a name="systemwide-options-for-the-current-user"></a>Au niveau système options pour l'utilisateur actuel
...
-   ** Préférences : ** Ici vous pouvez choisir des valeurs par défaut pour chaque fois que vous ouvrez Dynamics 365 pour les opérations, notamment la société, page d'origine, et la vue par défaut/mode d'édition (qui détermine si une page est verrouillée pour afficher et/ou ouvert pour vous modifier chaque fois ouvrez-le.) Vous trouverez également la langue, le fuseau horaire, la date, l'heure, et les options de format de numéro. Enfin, cette page contient de nombreuses préférences diverses qui varieront d'une version à l'autre.
...
**Colonnes de la grille :** vous pouvez régler la largeur d'une colonne dans la liste en sélectionnant la barre de calibrage à gauche ou à droite de l'en-tête de colonne et en la glissant à gauche ou à droite jusqu'à la largeur souhaitée. Dynamics 365 pour les opérations considère la largeur que vous souhaitez et l'indique que la colonne à la largeur chaque fois que vous ouvrez la page à cette liste. 

**Organisateurs :** certaines pages ont des sections extensibles appelées Organisateurs. Dynamics 365 pour les opérations s'considère que les organisateurs vous avez développées, et que les organisateurs vous ont réduites. Chaque fois que vous revenez à la page, ces mêmes organisateurs sont développés ou réduits comme la dernière fois que vous les avez utilisés. Dans cet article, nous expliquerons comment modifier l'ordre des sections de l'organisateur. Dans certains cas, réduire un organisateur peut améliorer les performances car Dynamics 365 pour les opérations n'a pas besoin d'extraire les informations de cet organisateur jusqu'à ce que l'organisateur est développé. 

**Récapitulatif :** certaines pages ont une section appelée volet Récapitulatif. Ce volet contient des informations en lecture seule associées au sujet actuel de la page. Chaque section dans le volet Récapitulatif est appelée un Récapitulatif. Vous pouvez développer ou réduire une zone récapitulative et un Dynamics 365 pour les opérations considère votre préférence. Dans certains cas, réduire une zone récapitulative peut améliorer les performances car Dynamics 365 pour les opérations n'a pas besoin de récupérer les informations relatives à ce zone récapitulative jusqu'à ce que la zone récapitulative est développé.
...
Chaque personne et société a un point de vue différent sur les données qui sont les plus importantes pour elle, ou sur les données qui ne sont pas nécessaires pour son travail. La capacité de travailler la manière vos données est classée, interagi avec, voire masquée est la clé à effectuer Dynamics 365 pour les opérations une expérience personnelle et productive. 

Les personnalisations explicites sont ces personnalisations que vous effectuez explicitement avec l'intention de modifier l'apparence ou le comportement d'un élément ou d'une page, en choisissant un menu de personnalisation. Le type le plus fondamental de personnalisation d'explication dans lequel vous cliquez avec le bouton droit sur un élément et sélectionnez ** personnalisez **. (Note que tous les éléments sur la page peut être personnalisée.) Lorsque vous sélectionnez cette méthode de personnalisation, vous consulterez la fenêtre de la propriété de l'élément. 

[fois![les propriétés d'un élément] (. /media/personalization-element-properties.jpg)](. /media/personalization-element-properties.jpg) 
...
[barre d'outils de personnalisation d'![] (. /media/personalization-personalizationtoolbar.jpg)](. /media/personalization-personalizationtoolbar.jpg)

La barre d'outils de personnalisation a un nombre d'actions de personnalisation. Choisissez l'outil **Sélectionner** lorsque vous souhaitez sélectionner et modifier les propriétés de plusieurs éléments, un par un. D'abord, cliquez sur l'outil Sélectionner, puis cliquez sur l'élément dont vous voulez modifier les propriétés. Lorsque vous sélectionnez un élément, la fenêtre de propriété de l'élément s'ouvre et vous pouvez modifier les propriétés de cet élément. Vous pouvez répéter le processus pour d'autres éléments dans l'écran qui sont personnalisables. Dans certains cas, vous sélectionnerez un élément et verrez que certaines propriétés ne sont pas modifiables. Cela signifie que la manière selon l'élément actuel est utilisée, Dynamics 365 pour les opérations ne peut pas vous laisser modifier cette propriété. Par exemple, vous ne pouvez pas masquer un champ obligatoire. 

Choisissez l'outil **Déplacer** lorsque vous souhaitez sélectionner et déplacer un élément dans un autre emplacement au sein du groupe actuel d'éléments. (Vous ne pouvez pas déplacer un élément en dehors de son groupe parent). D'abord, cliquez sur l'outil Déplacer, puis cliquez sur l'élément que vous voulez déplacer. Lorsque vous cliquez sur l'option que vous souhaitez déplacer, Dynamics 365 pour les opérations analyse l'écran pour inclure dans lesquels cet élément peut être déplacé et créer une série de « zones de dépôt » cette affiche comme ligne colorée et grasse en regard de la région où l'élément peut être déposé comme vous faites glisser l'élément le tour dans le groupe actuel. 

Choisissez l'outil **Masquer** pour sélectionner et masquer un élément. Pour masquer un élément, choisissez simplement l'outil Masquer et cliquez sur l'élément que vous voulez masquer. Lorsque vous sélectionnez l'outil Masquer, tous les éléments actuellement masqués sont rendus visibles et affichés dans un conteneur grisé afin de pouvoir choisir l'élément à afficher. Choisissez l'outil Sélectionner pour voir la manière dont la page s'affichera avec les éléments sélectionnés masqués. Choisissez l'outil **Récapitulatif** lorsque vous souhaitez qu'un champ numérique ou de chaîne s'affiche dans la zone récapitulative de l'organisateur. L'outil Récapitulatif s'applique uniquement aux champs contenus dans une section de l'organisateur. Lorsque vous choisissez l'outil de synthèse, Dynamics 365 pour les opérations affiche tous les champs sélectionnés comme champs de synthèse en les associant dans un conteneur estompé. Vous pouvez en mode interactif ajouter et supprimer des champs d'un récapitulatif de l'organisateur en cliquant sur le champ. 

Choisissez ** saut ** l'outil pour supprimer un élément de la séquence de l'onglet du clavier de la page. Lorsque vous choisissez l'outil de saut, tous les éléments actuel ignorés sont affichés dans un conteneur grisée afin de pouvoir les choisir de nouveau pour leur faire partie de la séquence de l'onglet en sélectionnant un élément supprimé. 
...
Choisissez ** gérez ** bouton se pour afficher une liste d'options de gestion relatives à toutes les personnalisations de la page actuelle. 

Choisissez ** espace libre ** de réinitialiser la page sur sa valeur par défaut, état installé. Toutes les personnalisations sur la page actuelle seront désactivées. Il existe aucun Annuler l'action, utilisent ce que cette option lorsque vous êtes certain que vous souhaitez réinitialiser la page. 
...
Avec la barre d'outils de personnalisation, l'enregistrement est implicite. Vos personnalisations prennent effet immédiatement à mesure que vous les faites et il n'y a pas besoin de cliquer sur un bouton **Enregistrer**. Dans certains cas, vous obtenez une icône du verrou en regard d'un élément lorsque vous sélectionnez un outil. Cela signifie que pour que la page fonctionne comme correctement, vous ne pouvez pas modifier les propriétés liées à l'outil sélectionné. Lorsque la barre d'outils de personnalisation est ouverte, la page devient non-interactive. Vous ne pouvez pas entrer des données ni développer ou réduire les sections.
...
Certains espaces de travail contiennent les vignettes de comptage (vignettes avec des numéros sur ces) que vous voudraient également afficher sur votre tableau de bord. Dans un espace de travail, cliquez avec le bouton droit sur une vignette de comptage et sélectionnez ** personnalisez **. Sélectionnez **Épingler au tableau de bord**. La prochaine fois que vous accéderez (et actualiserez) le tableau de bord sélectionné, vous verrez ce comptage en dessous de la vignette de navigation de cet espace de travail dans le tableau de bord.
