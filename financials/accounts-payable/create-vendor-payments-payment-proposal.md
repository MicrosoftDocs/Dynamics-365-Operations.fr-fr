...
La demande de proposition de paiement contient différents onglets, chacun ayant leurs options pour sélectionner les factures à payer. ** Paramètre ** l'onglet contient les options à une majorité d'utilisation d'organisation le plus souvent. Sous ** des enregistrements à inclure ** l'organisateur, vous pouvez indiquer quelles factures ou des fournisseurs à inclure pour le paiement en définissant des plages pour des spécifications différentes. Par exemple, si vous souhaitez payer uniquement une plage de fournisseurs, vous pouvez définir un filtre pour la plage de fournisseur. Cette fonctionnalité est souvent utilisée pour sélectionner des factures pour les détails d'un mode de paiement. Par exemple, si vous définissez un filtre dans lequel ** mode de paiement ** = ** chèque **, seules les factures associées à ce mode de paiement sont sélectionnées pour le paiement, si elles répondent également à d'autres critères spécifiés dans la requête. L'onglet **Paramètres avancés** contient des options supplémentaires (certaines d'entre elles peuvent ne pas concerner votre organisation). Par exemple, cet onglet contient les options pour payer les factures des paiements centralisés.

## <a name="parameters"></a>Paramètres
-   ** Sélectionner les factures de factures par ** – dans la plage de dates spécifiée par ** de la date ** et ** jusqu'à présent ** des champs peuvent être sélectionnés par date d'échéance, la date d'escompte de règlement, ou les deux. Si vous utilisez la date d'escompte de règlement, le système recherche d'abord les factures dont la date d'escompte de règlement entre de la Du et Au. Le système détermine ensuite si la facture est éligible pour l'escompte de règlement à l'aide de la date de la session pour garantir que la date d'escompte de règlement n'est pas déjà passée.
...
-   **Date de paiement minimal** – Permet d'entrer la date de paiement minimal. Par exemple, ** de la date ** et ** jusqu'à présent ** les champs spécifient une plage du 1er septembre au 10 septembre, et la date de paiement minimal est le 5 septembre. Dans ce cas, toutes les factures dont la date d'échéance du 1er septembre au 5 septembre avoir une date de paiement du 5 septembre. Toutefois, toutes les factures dont la date d'échéance du 5 septembre au 10 septembre avoir une date de paiement qui correspond à la date d'échéance de chaque facture.
-   **Montant limite** – Saisissez le montant total maximal pour tous les paiements.
-   ** Créer des paiements sans aperçu de facture ** – Si cette option est définie ** Oui **, les paiements sont créés immédiatement sur ** les paiements fournisseur ** la page. ** Proposition de paiement ** la page sera ignorée. Par conséquent, les paiements seront créés plus rapidement. Les paiements peuvent encore être modifiés à partir de la page **Paiements fournisseur**. Sinon, vous pouvez revenir sur la page **Proposition de paiement** à l'aide du bouton **Modifier les factures du paiement sélectionné**.

## <a name="advanced-options"></a>Options avancées
-   **Vérifier le solde fournisseur** – Si cette option est définie sur **Oui**, le système vérifie qu'un fournisseur n'a pas de solde débiteur avant le règlement de n'importe quelle facture. Si un fournisseur a un solde débiteur, aucun paiement n'est créé. Par exemple, le fournisseur peut avoir des avoirs, ou de paiements qui ont été validées mais pas encore réglés. Dans ces cas, le fournisseur ne doit pas être payé. Au lieu de cela, les avoirs ou les paiements doivent être réglés par rapport aux factures en attente.
...
-   **Paiement de la somme des factures** – Si l'option **Supprimer les paiements négatifs** est définie sur **Oui**, et si une facture et un paiement non réglés existent pour un fournisseur, les deux sont sélectionnés pour paiement et les montants sont ajoutés pour générer montant total du paiement. La seule exception se produit si la somme entraîne un remboursement. Dans ce cas, ni la facture ni le paiement n'est sélectionné. Si ** des paiements négatifs d'effacement ** l'option est définie ** aucun **, et une facture et un paiement ne sont pas réglées, la facture et au paiement sont sélectionnées pour le paiement, et les montants sont ajoutés simultanément pour produire le total des paiements.
...
-   ** La devise de paiement ** – Ce champ indique la devise que tous les paiements sont créés dans. Si une devise n'est pas définie, chaque facture est payée dans la devise de la facture.
...
-   ** Type de compte de contrepartie ** et ** compte de contrepartie ** – Permet de définir ces champs pour définir un type de compte spécifique (par exemple ** comptabilité ** ou ** banque **) et le compte de contrepartie (tel qu'un compte bancaire spécifique). Le mode de paiement pour la facture définit le type et le compte de contrepartie de compte de contrepartie par défaut, mais vous pouvez utiliser ces champs pour remplacer les valeurs par défaut.
-   ** Les filtres supplémentaires ** – Sous ** des enregistrements à inclure ** l'organisateur, vous pouvez définir les plages supplémentaires des critères. Par exemple, si vous souhaitez payer qu'une plage de fournisseurs, vous pouvez définir un filtre pour la plage de fournisseur. Cette fonctionnalité est souvent utilisée pour sélectionner des factures pour les détails d'un mode de paiement. Par exemple, si vous définissez un filtre dans lequel ** mode de paiement ** = ** chèque **, seules les factures associées à ce mode de paiement sont sélectionnées pour le paiement, si elles répondent également à d'autres critères spécifiés dans la requête.
...
April sélectionne **Escompte de règlement** comme type de proposition. Elle entre une plage de dates du du 26 juin au 10 juillet. Les factures suivantes sont incluses dans la proposition :
...
April sélectionne **Par date d'échéance** comme type de proposition. Elle entre une plage de dates du du 26 juin au 10 juillet. Les factures suivantes sont incluses dans la proposition :
...
April sélectionne **Échéance et escompte de règlement** comme type de proposition. Elle entre une plage de dates du du 26 juin au 10 juillet. Les factures suivantes sont incluses dans la proposition :
...
-   Le champ **Contrôle de dimension** est activé et d'autres dimensions sont activées. Maintenant vous définissez comment les dimensions seront copiées dans le journal. Par exemple : • Sélectionnez ** BusinessUnit ** la case à cocher pour créer une proposition de paiement par unité commerciale pour le mode de paiement, • Sélectionnez ** CostCenter ** la case à cocher pour créer une proposition de paiement par centre de coût pour le mode de paiement
