...
-   ** Administration d'escompte de règlement ** – Permet de spécifier comment a [l'escompte de règlement est gérée lorsqu'une facture est sur-payée] (cash-discount-handling-overpayments.md). Pour un trop-perçu, l'escompte de règlement peut être réduite, elle peut être considérée comme une différence, ou elle peut rester activée pour le fournisseur ou le client.
...
-   **Calculez les escomptes de règlement pour les avoirs** – Définissez cette option sur **Oui** pour calculer automatiquement un escompte de règlement pour les avoirs. Dans Comptabilité client, une transaction d'avoir est une transaction négative ayant une valeur dans le champ **Facture** de la page **Facture financière** ou un retour dans la page **Commande client**.
    -   L'effet de cette option dépend de la valeur du champ **Utiliser un escompte de règlement** dans la page **Régler les transactions**. Si cette option est définie ** Oui **, la remise est prélevée lorsque le champ de **** d'escompte de règlement d'utilisation de **** est défini ** normale **. Lorsque le champ de **** d'escompte de règlement d'utilisation de **** est défini ** toujours **, l'escompte de règlement est toujours prélevé, quel que soit le paramètre de ce champ. Lorsque le champ de **** d'escompte de règlement d'utilisation de **** est défini ** jamais **, l'escompte de règlement n'est jamais prélevé, quel que soit le paramètre de ce champ.
...
-   **Classer le règlement par ordre de priorité (AR uniquement)** – Définissez cette option sur **Oui** pour activer le bouton **Marque par priorité** dans les pages **Entrer les paiements client** et **Régler les transactions**. Ce bouton permet aux utilisateurs affecter l'ordre de règlement prédéterminé aux transactions.  Une fois l'ordre de règlement a été appliqué à une transaction, l'ordre et la répartition des paiements peuvent être modifiées avant la validation.
-   ** Priorité d'utilisation pour les règlements automatiques ** – définissez cette option ** Oui ** d'utiliser l'ordre de priorité défini lorsque les transactions sont automatiquement réglées. Ce champ n'est disponible que si ** hiérarchisez le règlement ** et ** règlement automatique ** des options sont définies ** Oui **.
