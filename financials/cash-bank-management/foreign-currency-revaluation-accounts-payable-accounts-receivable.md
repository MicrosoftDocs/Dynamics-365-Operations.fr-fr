...
La valeur théorique (valeur comptable) des transactions fournisseur en cours en devises étrangères varie avec le temps en raison des fluctuations des taux de change. Pour mettre à jour la valeur des transactions en cours dans Ventes et Achats, exécutez le processus de réévaluation des comptes en devises. La réévaluation des comptes en devises peut être exécutée à la fois pour les Achats et les Ventes. Le processus utilise un nouveau taux de change pour réévaluer les montants en cours, ou les montants non réglés, à une date particulière. Les différences entre l'origine validés les montants et les montants réévalués provoquent un profit non réalisé ou une perte pour chaque transaction en cours. Les comptabilités auxiliaires Achats et Ventes sont ensuite mis à jour pour refléter les profits non réalisés ou la perte, et une écriture comptable est validée dans la comptabilité.
...
Utilisez ** réévaluation des comptes en devises ** la page sous ** des tâches périodiques ** pour réévaluer des transactions en cours. Vous pouvez exécuter le processus en temps réel ou le replanifier à l'aide d'un traitement par lots. Lorsque vous définissez des paramètres pour le processus de réévaluation, assurez-vous de vérifier si vous souhaitez imprimer un état des résultats. L'état de réévaluation ne peut pas être réimprimé une fois le processus terminé. Si vous générez l'état de réévaluation des comptes en devises, il affiche divers soldes au niveau du client/fournisseur et au niveau de la devise :
