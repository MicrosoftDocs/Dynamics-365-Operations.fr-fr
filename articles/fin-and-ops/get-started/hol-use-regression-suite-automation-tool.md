...
1. Ouvrez l'outil RSAT (Regression Suite Automation Tool), puis sélectionnez les deux scénarios de test que vous avez créés dans [Configurer et installer l'outil Regression Suite Automation Tool](./hol-set-up-regression-suite-automation-tool.md).
...
Cette fonction peut être utilisée pour vérifier si une action s'est produite. Par exemple, lorsqu'un ordre de fabrication est créé, estimé, puis démarré, Microsoft Dynamics 365 for Finance and Operations présente un message « Production – démarrer » pour vous informer que l'ordre de fabrication a commencé.
...
Une fois le scénario de test exécuté, le message du fichier de paramètres Excel est comparé au message qui s'affiche dans Finance and Operations. Si les messages ne correspondent pas, le scénario de test échoue.
