---
title: Obtenir de l'aide sur Talent
description: "Cette rubrique explique comment obtenir de l'aide sur Dynamics 365 for Talent."
author: Darinkramer
manager: AnnBe
ms.date: 09/18/2018
ms.topic: article
ms.prod: 
ms.service: dynamics-365-talent
ms.technology: 
audience: Application User
ms.reviewer: josaw
ms.search.scope: Talent
ms.search.region: Global
ms.author: dkrame
ms.search.validFrom: 2018-09-30
ms.dyn365.ops.version: Talent September 10, 2018 update
ms.translationtype: HT
ms.sourcegitcommit: 1bf37d65cd8ce6a98fc2d2fb11ae9587cf6958a3
ms.openlocfilehash: 9b2d7159b458b6f3b2cbb3271210c10ab486f496
ms.contentlocale: fr-fr
ms.lasthandoff: 09/27/2018

---

# <a name="get-support-for-talent"></a>Obtenir de l'aide sur Talent

[!include [banner](includes/banner.md)]

Cette rubrique explique comment obtenir de l'aide sur Talent : Core HR, Attract et Onboard.

<a name="contact-support"></a>Contacter le support
---------------

Utilisez les informations contenues dans cett rubrique pour contacter Microsoft si vous avez des questions techniques sur Talent.

### <a name="talent-support---core-hr"></a>Support pour Talent - Core HR

Pour obtenir de l'aide sur Talent, utilisez les ressources du tableau suivant. Ces ressources sont répertoriées dans l'ordre de suivi recommandé lorsque vous résolvez des problèmes.

| **Tâche**                                                | **Informations supplémentaires**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|---------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Demander à la communauté                                      | Accédez à la page [Demander à la communauté](https://community.dynamics.com/365/talent) pour poser vos questions, et utilisez l'option « Aide » pour rechercher des rubriques dans Core HR.                                                                                                                                                                                                                                                                                                                                                                                  |
| Utiliser l'outil Support cloud                     | Dans [Lifecycle Services (LCS)](https://lcs.dynamics.com/), la vignette **Support** est un outil qui vous aide à gérer les incidents de support. Pour soumettre vos problèmes directement à Microsoft, accédez à la vignette **Support** dans votre projet LCS ou utilisez la section **Tickets de support** dans LCS et cliquez sur **Support cloud**. Vous pouvez soumettre des problèmes de deux manières. 1 : lorsque vous soumettez un problème via LCS, sous l'onglet **Problème actif**, sélectionnez votre problème, puis cliquez **Soumettre à Microsoft**. 2 : sous l'onglet **Soumis à Microsoft**, cliquez sur **Soumettre un nouvel incident** pour soumettre de nouveaux problèmes. |
| Poser des questions sur les licences Talent         | Contactez votre partenaire ou votre représentant commercial Microsoft.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Demander de nouvelles fonctionnalités dans Talent | Sur la page principale Talent de l'application, cliquez sur l'icône **?** et sélectionnez **Idées**, ou utilisez le lien [Idées](https://powerusers.microsoft.com/t5/Ideas-for-Human-Resources/idb-p/HumanResources) pour afficher, rechercher, voter ou ajouter de nouvelles idées.                                                                                                                                                                                                                                                                                                                      |

### <a name="talent-support--attract-and-onboard"></a>Support pour Talent – Attract et Onboard

Pour obtenir de l'aide sur Attract et Onboard, utilisez les ressources du tableau suivant. Ces ressources sont répertoriées dans l'ordre de suivi recommandé lorsque vous résolvez des problèmes. Toutes les options de support sont disponibles sous l'icône **?** dans la zone supérieure droite du produit.

| **Tâche**                                                                | **Informations supplémentaires**                                                                                                                                                                                                                                                                           |
|-------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Demander à la communauté                                                      | Accédez à la page [Demander à la communauté](https://community.dynamics.com/365/talent) pour poser vos questions.                                                                                                                                                                                |
| Signaler un problème                                                        | Sélectionnez l'icône **?** dans le menu et sélectionnez les options pour signaler un problème. Décrivez le problème et indiquez une adresse électronique si vous souhaitez suivre le problème signalé.                                                                                           |
| Poser des questions sur les licences Talent                         | Contactez votre partenaire ou votre représentant commercial Microsoft.                                                                                                                                                                                                                                        |
| Demander de nouvelles fonctionnalités dans Attract et Onboard | Cliquez sur l'icône **?** et sélectionnez **Idées**, ou utilisez les liens suivants pour afficher, rechercher, voter ou ajouter de nouvelles idées : [Attract](https://powerusers.microsoft.com/t5/Ideas-for-Attract/idb-p/Attract) ou [Onboard](https://powerusers.microsoft.com/t5/Ideas-for-Onboard/idb-p/Onboard). |

