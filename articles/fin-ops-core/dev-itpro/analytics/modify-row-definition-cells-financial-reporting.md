...
Dans l'exemple suivant, Phyllis souhaite empêcher que le titre et les traits de soulignement dans la ligne **Disponibilités totales** de son état soient imprimés, car il n'y a eu aucune activité dans aucun des comptes de disponibilités. Par conséquent, dans la ligne 220 (qui est une ligne de format comme l'indique le code de format **---**), dans la cellule **Formules/Lignes/Unités associées**, elle entre **250**, qui est le code de ligne de la ligne de montant qu'elle souhaite supprimer.
