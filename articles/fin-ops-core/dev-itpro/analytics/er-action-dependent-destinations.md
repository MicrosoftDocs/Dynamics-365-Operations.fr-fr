...
Vous pouvez configurer les [destinations](electronic-reporting-destinations.md) pour chaque composant (dossier ou fichier) d’une [configuration](general-electronic-reporting.md#Configuration) de [format](general-electronic-reporting.md#FormatComponentOutbound) de [gestion des états électroniques](general-electronic-reporting.md) utilisée pour générer un document sortant. Les utilisateurs qui exécutent un format pour la gestion des états électroniques de ce type, et qui ont les droits d’accès appropriés, peuvent également changer les paramètres de destination configurés au moment de l’exécution.
