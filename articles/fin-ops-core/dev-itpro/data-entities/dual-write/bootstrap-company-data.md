...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="35f4e-103">Démarrer la FAQ relative aux données de la société</span><span class="sxs-lookup"><span data-stu-id="35f4e-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="35f4e-104">Pourquoi ai-je besoin de procéder à un démarrage ?</span><span class="sxs-lookup"><span data-stu-id="35f4e-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="35f4e-105">Vous devriez avoir un Common Data Service existant ou autre instance d'application Dynamics 365 avec des données commerciales, et vous devez activer la connexion en double écriture.</span><span class="sxs-lookup"><span data-stu-id="35f4e-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="35f4e-106">Dans ce cas, vous devez démarrer Common Data Service ou d'autres données de l'application Dynamics 365 avec les informations de l'entreprise avant d'activer la connexion en double écriture.</span><span class="sxs-lookup"><span data-stu-id="35f4e-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="35f4e-107">Quand dois-je utiliser le démarrage ?</span><span class="sxs-lookup"><span data-stu-id="35f4e-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="35f4e-108">Vous devez utiliser le démarrage avant d'activer les cartes d'entité en double écriture (pendant l'étape n° 5).</span><span class="sxs-lookup"><span data-stu-id="35f4e-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="35f4e-109">Pour configurer la connexion en double écriture entre les instances de votre application Finance and Operations et de l'application Common Data Service ou de toute autre application Dynamics 365, connectez-vous à l'application Finance and Operations en tant qu'administrateur.</span><span class="sxs-lookup"><span data-stu-id="35f4e-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="35f4e-110">Accédez au module **Gestion des données** et cliquez sur le bouton **Double-écriture**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="35f4e-111">Cela lance l' **Intégrateur de données**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="35f4e-112">Créez la connexion en double écriture pour une ou plusieurs sociétés.</span><span class="sxs-lookup"><span data-stu-id="35f4e-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="35f4e-113">![Créer la connexion en double écriture](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="35f4e-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="35f4e-114">Activez la carte d'entité **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="35f4e-115">Cela synchronise les entreprises de l'application Finance and Operations pour Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="35f4e-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="35f4e-116">![Activer la carte d'entité](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="35f4e-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="35f4e-117">Exécutez les exemples de code de démarrage sur Common Data Service ou toute autre instance d'application de Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="35f4e-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="35f4e-118">Une fois le démarrage effectué et le système prêt pour la synchronisation en direct, activez les cartes d'entité.</span><span class="sxs-lookup"><span data-stu-id="35f4e-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="35f4e-119">L'activation des cartes d'entité déclenche la synchronisation des données initiales pour les cartes d'entité activées.</span><span class="sxs-lookup"><span data-stu-id="35f4e-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="35f4e-120">Les données correspondant aux sociétés sélectionnées sur la connexion en double écriture sont synchronisées entre l'application Finance and Operations et Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="35f4e-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="35f4e-121">Comment utiliser l'exemple de code ?</span><span class="sxs-lookup"><span data-stu-id="35f4e-121">How to I use the code sample?</span></span>
<span data-ttu-id="35f4e-122">L'exemple de code est une application C# que vous pouvez charger dans Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="35f4e-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="35f4e-123">Il prend des dépendances de pack NuGet sur le kit de développement logiciel Common Data Service, que vous pouvez actualiser via l'outillage standard de Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="35f4e-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="35f4e-124">Après avoir décompressé et ouvert la solution dans Visual Studio et avoir restauré les packs NuGet, recherchez **TODO** dans le code.</span><span class="sxs-lookup"><span data-stu-id="35f4e-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="35f4e-125">Chaque décision que vous devez prendre sur la manière dont vous souhaitez démarrer les informations d'entreprise est indiquée par un **TODO** , avec un exemple de code pour mise en place canonique.</span><span class="sxs-lookup"><span data-stu-id="35f4e-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO** , with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="35f4e-126">L'exemple de code n'affiche qu'une des nombreuses façons dont vous pouvez catégoriser les enregistrements d'entité par entreprise.</span><span class="sxs-lookup"><span data-stu-id="35f4e-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="35f4e-127">En modifiant la logique dans les sections **TODO** , vous pouvez créer votre catégorisation personnalisée.</span><span class="sxs-lookup"><span data-stu-id="35f4e-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="35f4e-128">À quoi dois-je m'attendre ?</span><span class="sxs-lookup"><span data-stu-id="35f4e-128">What should I expect?</span></span>
<span data-ttu-id="35f4e-129">Par défaut, l'exemple d'application vous permet de fournir un dictionnaire de mises en correspondance de code d'unité commerciale vers entreprise.</span><span class="sxs-lookup"><span data-stu-id="35f4e-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="35f4e-130">Une entité que vous démarrez avec un champ **OwningBusinessUnit** est automatiquement définie pour utiliser la société spécifiée.</span><span class="sxs-lookup"><span data-stu-id="35f4e-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="35f4e-131">Une entité sans champ **OwningBusinessUnit** , comme un produit, définira la société selon la mise en correspondance avec une valeur d'unité commerciale vide.</span><span class="sxs-lookup"><span data-stu-id="35f4e-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="35f4e-132">L'application de la console anticipe un paramètre, **–simulate** ou **–apply**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="35f4e-133">Si vous utilisez le paramètre de ligne de commande **–simulate** , aucune donnée n'est mise à jour.</span><span class="sxs-lookup"><span data-stu-id="35f4e-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="35f4e-134">Seuls les fichiers **<entityname>.csv** sont générés dans le même dossier que l'outil, un pour chaque entité qui aurait été mise à jour.</span><span class="sxs-lookup"><span data-stu-id="35f4e-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="35f4e-135">Vous pouvez itérativement examiner ces fichiers tout en faisant en sorte de veiller à ce que le code mette à jour les valeurs d'entreprise, comme prévu.</span><span class="sxs-lookup"><span data-stu-id="35f4e-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="35f4e-136">Lorsque vous terminez les mises à jour simulées, alors utilisez le paramètre **–apply**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="35f4e-137">Cela met à jour tous les enregistrements qui ont actuellement une valeur d'entreprise incorrecte, en lots de 1 000 enregistrements à la fois (par défaut).</span><span class="sxs-lookup"><span data-stu-id="35f4e-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="35f4e-138">Le code est idempotent comme indiqué. Autrement dit, vous pouvez le réexécuter et seules les entreprises mal affectées seront mises à jour.</span><span class="sxs-lookup"><span data-stu-id="35f4e-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="35f4e-139">Lors de l'exécution avec **–apply** , le code génère des fichiers CSV des modifications apportées, qui sont intitulés **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="35f4e-139">When running with **–apply** , the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
