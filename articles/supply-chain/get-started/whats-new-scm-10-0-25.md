...
| Approvisionnements | Désactiver le bouton de réinitialisation de la répartition des demandes d’achat | Cette fonction désactive le bouton **Réinitialiser** sur la page **Répartition comptable** pour les demandes d’achat en cours de révision. |
...
| Gestion des entrepôts | Désactiver les réceptions prévues à partir d’ordres de qualité qui échantillonnent le stock bloqué | Cette fonctionnalité empêche le système de créer des transactions de réception attendues pour les ordres de qualité qui échantillonnent le stock avec un statut bloquant. Étant donné que l’inventaire bloqué n’est pas disponible, cette fonctionnalité supprime les réceptions attendues. Cela permet de s’assurer que l’inventaire ne se termine pas avec plusieurs statuts de blocage, ce qui peut entraîner des problèmes d’intégrité des données. |
...
| --- | --- | --- |
| Gestion des actifs | Appliquer des règles pour le regroupement des ordres d’exécution lors de l’exécution d’un plan de maintenance | Disponibilité générale |
| Gestion des actifs | Améliorations apportées à la maintenance par compteur | Disponibilité générale |
| Gestion des coûts | Niveau de calcul des coûts | Disponibilité générale |
...
| Gestion des coûts | Détails de la progression de la clôture du stock | Disponibilité générale |
| Gestion des coûts | Options des dimensions financières par défaut pour la réévaluation du coût standard du stock | Disponibilité générale |
| Gestion des coûts | Nettoyage des données de l’état de valeur de stock | Activé par défaut |
| Gestion des coûts | Stockage de l’état de valeur de stock | Activé par défaut |
| Gestion des coûts | Afficher le journal de clôture dans la grille | Activé par défaut |
| Gestion des modifications d’ingénierie | Activer la gestion des changements sur les produits existants | Activé par défaut |
| Gestion des modifications d’ingénierie | Gestion des modifications d’ingénierie | Activé par défaut |
| Gestion des modifications d’ingénierie | Notifications d’ingénierie pour la production | Activé par défaut |
| Gestion des modifications d’ingénierie | Amélioration de l’héritage des attributs pour la gestion des changements d’ingénierie | Activé par défaut |
| Gestion des modifications d’ingénierie | Gérer les modifications apportées aux formules et à leurs ingrédients | Activé par défaut |
| Gestion des modifications d’ingénierie | Vérifications de la disponibilité du produit | Activé par défaut |
| Gestion des modifications d’ingénierie | Générer des variantes pour les produits d’ingénierie | Activé par défaut |
...
| Planification générale | Confirmation et consolidation pouvant être traitées par lots pour les lots de commandes planifiées de consommables et d’articles emballés | Disponibilité générale |
...
| Planification générale | Sélection du modèle de prévision sur les détails de prévision de la demande | Obligatoire |
| Planification générale | Visualisation de la progression de la planification | Obligatoire |
| Planification générale | Confirmation de la mise en parallèle des ordres prévisionnels | Obligatoire |
| Planification générale | Confirmation de l’ordre planifié avec filtrage | Activé par défaut |
| Planification générale | Vues enregistrées des ordres prévisionnels | Activé par défaut |
...
| Approvisionnements | Activer la réinitialisation des flux de travail liés à l’approvisionnement | Disponibilité générale |
...
| Approvisionnements | Autoriser les fournisseurs à appliquer les catégories d’approvisionnement via la collaboration fournisseur | Activé par défaut |
...
| Approvisionnements | Partie responsable du contrat d’achat | Activé par défaut |
| Approvisionnements | Vues enregistrées pour les commandes fournisseur | Activé par défaut |
| Gestion des informations sur les produits | Contrôle stricte des quantités de commande par défaut | Obligatoire |
...
| Gestion des informations sur les produits | Améliorations de la page de suggestions de variante | Activé par défaut |
...
| Contrôle de la production | Activer la génération automatique du numéro de contenant lors de la déclaration de fin dans le périphérique de bon de travail | Obligatoire |
...
| Contrôle de la production | Fonctionnalité de verrouillage des périphériques des bons de travail et des terminaux des bons de travail afin de procéder à leur désinfection | Obligatoire |
...
| Contrôle de la production | Contenant pour la déclaration ajouté comme finalisé au périphérique pour bons de travail | Obligatoire |
| Contrôle de la production | Imprimer une étiquette à partir d’un périphérique de bons de travail | Obligatoire |
| Contrôle de la production | Exécution de l’atelier de production | Obligatoire |
| Contrôle de la production | Fonctionnalité de gestion d’actifs pour l’interface d’exécution de l’atelier de production | Activé par défaut |
| Contrôle de la production | Recherche de tâche pour l’interface d’exécution de l’atelier de production | Activé par défaut |
| Contrôle de la production | Remplacer la réservation de production par défaut | Activé par défaut |
| Contrôle de la production | Afficher les numéros de série, de traitements par lots et de contenants complets dans l’interface d’exécution de l’atelier de production | Activé par défaut |
| Ventes et marketing | Amélioration des performances des commandes client | Disponibilité générale |
...
| Ventes et marketing | Stratégie de suppression entre les lignes de commande client et lignes de commande fournisseur | Obligatoire |
| Ventes et marketing | Stratégie d’exportation de données référencées de devis de vente | Obligatoire |
| Ventes et marketing | Optimisation de l’exportation de l’entité de données des contacts | Activé par défaut |
...
| Ventes et marketing | Améliorer les performances de l’état des « 100 principaux clients » | Activé par défaut |
| Ventes et marketing | Recalculer le solde client estimé | Activé par défaut |
| Ventes et marketing | Enregistrement de la ligne de commande client de retour avec précision décimale avec et sans poids variable | Activé par défaut |
| Ventes et marketing | Vues enregistrées pour les ventes et le marketing | Activé par défaut |
| Ventes et marketing | Confirmation de commande client en un seul clic | Activé par défaut |
| Gestion des entrepôts | Modèles de cross-docking avec instructions d’emplacement | Disponibilité générale |
| Gestion des entrepôts | Désactiver les réceptions prévues à partir d’ordres de qualité qui échantillonnent le stock bloqué | Disponibilité générale |
| Gestion des entrepôts | Historique de réception de contenant | Disponibilité générale |
| Gestion des entrepôts | Arrondir les quantités vers le bas pour l’unité de vente la plus proche à la libération dans l’entrepôt | Disponibilité générale |
...
| Gestion des entrepôts | Utiliser une API plus rapide pour la clôture/réouverture des conteneurs sur la station de conditionnement | Disponibilité générale |
| Gestion des entrepôts | Valider les modèles sélectionnés pour les tâches de réapprovisionnement | Disponibilité générale |
...
| Gestion des entrepôts | Modifier le statut du stock des articles contrôlés par les dimensions de suivi | Obligatoire |
| Gestion des entrepôts | Modifier le pool de travail sur le travail | Obligatoire |
| Gestion des entrepôts | Poste de groupement complet | Obligatoire |
| Gestion des entrepôts | Fonction de groupement consolidé | Obligatoire |
| Gestion des entrepôts | Confirmer et transférer | Obligatoire |
| Gestion des entrepôts | Confirmer les expéditions sortantes des traitements par lots | Obligatoire |
| Gestion des entrepôts | Contrôler l’affichage ou non d’une page de résumé de réception sur les appareils mobiles | Obligatoire |
| Gestion des entrepôts | Traitement différé de l’opération manuelle de mouvement de stock | Obligatoire |
...
| Gestion des entrepôts | Mises en page d’étiquettes de contenant améliorées | Obligatoire |
| Gestion des entrepôts | Évaluer toutes les actions pour les directives d’emplacement qui autorise plusieurs SKU | Obligatoire |
...
| Gestion des entrepôts | Positionnement des contenants d’emplacement | Obligatoire |
| Gestion des entrepôts | Mélange de dimensions de produit d’emplacement | Obligatoire |
...
| Gestion des entrepôts | Empêcher l’utilisation des contenants expédiés de l’ordre de transfert dans d’autres entrepôts que celui de destination | Obligatoire |
| Gestion des entrepôts | Inviter à résoudre les noms ambigus « Emplacement/Contenant » | Obligatoire |
| Gestion des entrepôts | Contrôle qualité | Obligatoire |
| Gestion des entrepôts | Paramètres utilisateur, icônes et titres des étapes pour la nouvelle application d’entrepôt | Obligatoire |
| Gestion des entrepôts | Zone d’emplacement supplémentaire | Activé par défaut |
| Gestion des entrepôts | Créer et traiter les ordres de transfert à partir de l’application d’entrepôt | Activé par défaut |
| Gestion des entrepôts | Activer le contrôle rapide pour les appareils mobiles d’entrepôt | Activé par défaut |
| Gestion des entrepôts | Utilisation de l’emplacement de consolidation des articles | Activé par défaut |
| Gestion des entrepôts | Durée d’exécution maximale de la tâche de nettoyage des entrées disponibles pour la gestion des entrepôts | Activé par défaut |
| Gestion des entrepôts | Visualisation de la charge de travail sortante | Activé par défaut |
| Gestion des entrepôts | Traiter les événements d’application d’entrepôt | Activé par défaut |
| Gestion des entrepôts | Vue enregistrée pour l’atelier de planification des chargements | Activé par défaut |
| Gestion des entrepôts | Vue enregistrée pour la page des détails du travail | Activé par défaut |
| Gestion des entrepôts | Vue enregistrée pour le traitement de la vague | Activé par défaut |
| Gestion des entrepôts | Vues enregistrées pour le traitement du chargement | Activé par défaut |
| Gestion des entrepôts | Vues enregistrées pour le traitement de l’expédition | Activé par défaut |
| Gestion des entrepôts | Détails du traitement par lots des vagues | Activé par défaut |
| Gestion des entrepôts | Notifications d’exécution de vague | Activé par défaut |
