...
1. Le fournisseur est informé de la livraison prévue. Cela peut se produire de l’une des trois manières suivantes :
...
1. Le stock est livré de US-104 à USMF.
1. Lorsque le matériel arrive chez USMF, la commande de réapprovisionnement avec consignation est mise à jour avec un accusé de réception de marchandises. Seuls les quantités physiques du stock appartenant au fournisseur sont enregistrées. Aucune transaction comptable n’est créée, car le stock appartient encore au fournisseur.
1. Le fournisseur supervise les mises à jour du stock physique disponible sur la page **Stock de consignation disponible**.
1. Maintenant que le stock physique est disponible, le processus de production réserver le stock appartenant au fournisseur et commence l’ordre de fabrication pour les produits finis qui vont consommer la matière première M9211CI.
1. Le propriétaire des matières premières réservées qui vont être consommées dans la production du jour passe d’US-104 à USMF. Cela s’est effectue à l’aide d’un journal des modifications de propriété du stock. Ce processus crée des commandes fournisseur dont le champ **Origine** est défini sur **Consignation**.
1. Le fournisseur surveille la consommation (transfert de propriété) sur la page **Produits reçus à partir du stock de consignation** et sort une facture basée sur les accords entre les deux sociétés.
1. Le processus de production consomme la matière première en fonction du prélèvement de production. La réservation physique est automatiquement mise à jour pour refléter que le stock disponible appartient à USMF.
1. La facture d’US-104 est traitée par rapport aux commandes fournisseur générées automatiquement lorsque le journal de modification de propriété du stock a été traité. Le paiement est effectué au fournisseur US-104 pour le stock consommé.
...
Le journal **Modifications de propriété du stock** est utilisé pour enregistrer le transfert de propriété du stock de consignation du fournisseur à l’entité juridique qui le consomme. Aucun mouvement de stock prévu n’est créé pour le journal. Comme tout autre journal de stock, il doit être identifié avec un nom de journal de stock. Ces noms sont créés sur la page **Noms des journaux de stock** et **Type de journal** doit être défini sur **Modification de propriété**.

Les seules transactions de stock créées sont celles qui sont associées à un journal validé. Quand le journal est-il validé :
...
Si vos fournisseurs utilisent l’interface de collaboration fournisseur, ils peuvent utiliser cette option pour contrôler la consommation du stock sur votre site. L’interface de collaboration fournisseur comporte trois pages associées au processus de consignation entrante :
