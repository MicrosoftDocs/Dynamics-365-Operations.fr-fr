...
Les codes de type de coût déterminent le type de coût encouru quand les marchandises sont débarquées dans l’entrepôt ou les coûts au débarquement du voyage. Bien qu’ils augmentent généralement la valeur des marchandises, ils peuvent également être utilisés pour enregistrer des montants dans la comptabilité. Les ajustements comptables sont utilisés quand un coût est comptabilisé au fil du temps ou au cours d’une série de voyages et compensé en une seule transaction.
...
| Compte d’écart | Le compte qui sera utilisé pour compenser les charges à payer au moment de la validation de la facture d’achat. Ce champ est utilisé uniquement quand l’option **Utiliser le compte de provisionnement des frais de type de coût** est définie sur *Oui* sur le raccourci **Coût** sur l’onglet **Général** de la page **Paramètres de coût au débarquement**. |
...
| Mode de livraison | Sélectionnez le mode de livraison que le modèle de coût doit utiliser au moment du calcul du coût estimé d’un article. Ce champ permet de déterminer les coûts automatiques associés aux marchandises sur l’estimation des coûts. |
...
Les diviseurs volumétriques sont utilisés pour calculer le poids volumétrique. Chaque entreprise d’expédition/de fret formule ses propres diviseurs volumétriques. De plus, les diviseurs d’une entreprise varient généralement en fonction du mode de livraison. Par exemple, l’air et la mer ont souvent des diviseurs très différents. Une entreprise peut également rendre ses règles plus complexes, en fonction de son origine. Le système utilise la formule suivante pour trouver le poids volumétrique : VolumetricWeight = Volume ÷ VolumetricDivisor.

Par exemple, un colis envoyé par avion a un volume de 3 mètres cubes (m³). L’entreprise facture en fonction du poids volumétrique et applique un diviseur volumétrique de 6. Ce diviseur est divisé par le volume pour déterminer le poids volumétrique. Par conséquent, le poids volumétrique pour cet exemple est de 3 ÷ 6 = 0,5 kilogrammes (kg).
...
| Diviseur volumétrique | Entrez la valeur du diviseur volumétrique qui s’applique à la ligne. Le volume de chaque colis sera divisé par la valeur que vous entrez ici pour déterminer le poids volumétrique de ce colis. |

> [!NOTE]
> Le système utilisera la valeur maximale entre **poids réel** et **poids volumétrique**.
