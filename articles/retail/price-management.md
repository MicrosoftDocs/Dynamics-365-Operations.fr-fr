...
Cette rubrique fournit des informations sur le processus de création et de gestion des prix de vente dans Dynamics 365 Retail. Elle se concentre sur les concepts impliqués dans ce processus, et sur les effets des différentes options de configuration sur les prix de vente.
...
Le prix de base et le prix d'accord commercial font partie des fonctionnalités de base de Microsoft Dynamics 365, et sont disponibles même si vous n'utilisez pas Retail. La fonctionnalité d'ajustement de prix est disponible uniquement dans Retail. La section suivante fournit d'autres informations sur chacune de ces options pour définir des prix et explique comment les options fonctionnent ensemble.
...
Si la devise d'un canal de vente au détail est différente de la devise de la société, le prix de base dans ce canal de vente au détail est déterminé à l'aide de la conversion de devise du prix défini dans le produit.

Bien que l'unité du prix ne soit pas un scénario de vente au détail courant, le moteur de la tarification de vente au détail la prend en charge. Si l'unité de prix est définie sur une valeur autre que **0** (zéro), le prix unitaire est égal à Prix ÷ Unité du prix. Par exemple, si le prix d'un produit est de 10,00 $, et que l'unité du prix est 50, le prix pour une quantité de 1 est de 0,20 $ (= 10,00 $ ÷ 50).
...
Un accord commercial de prix de vente **Groupe** est le type qui est le plus souvent utilisé avec la fonctionnalité de la vente au détail. En dehors de Retail, les accords commerciaux de prix de vente **Groupe** sont pour un groupe de clients spécifique. Toutefois, dans Retail, le concept de groupe de clients a été étendu afin que ce soit un groupe de prix de vente au détail plus générique. Un groupe de prix peut être lié à un canal de vente au détail, à une affiliation, à un programme de fidélité, ou à un catalogue. Pour plus d'informations sur les groupes de prix, voir la section « Groupes de prix » plus haut dans cette rubrique.
