...
description: Cette rubrique décrit l'utilisation de blocages sur les commandes à l'aide de Dynamics 365 Retail.
author: josaw1
manager: AnnBe
ms.date: 05/14/2018
ms.topic: article
...
Cette rubrique décrit les fonctions de blocage de commande de Dynamics 365 Retail pour les commandes du centre d'appels.
...
Pour créer un ensemble facultatif de codes motif de blocage, accédez à **Vente au détail** \> **Paramétrage du canal** \> **Codes info**. Ces codes info peuvent être utilisés comme code motif secondaire pour définir davantage le code de blocage principal. Sélectionnez **Nouveau** pour créer un ensemble de codes motif, puis sélectionnez **Sous-codes** pour définir la liste des motifs supplémentaires. Pour associer les codes info que vous définissez au canal du centre d'appels, accédez à **Vente au détail** \> **Canaux** \> **Centres d'appels** \> **Tous les centres d'appels**. Dans l'organisateur **Général**, définissez le champ **Code de blocage**.

## <a name="putting-orders-on-hold"></a>Mise en attente de commandes

Les commandes créées par les utilisateurs du centre d'appels dans le programme Retail administratif peuvent être mises en attente manuellement ou automatiquement dans des cas spécifiques.
...
Pour afficher toutes les commandes mises en blocage manuel ou automatique, accédez à **Vente au détail** \> **Clients** \> **Blocage de commandes**.
