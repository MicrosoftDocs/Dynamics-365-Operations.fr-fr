...
> [!NOTE]
> Sur un système à ordinateur unique tel qu’une topologie de développeur ou un environnement de démonstration, ou lorsque Commerce Scale Unit et Modern POS sont installés sur le même ordinateur, il est possible que Store Commerce ne puisse pas terminer l’activation de l’appareil. Ce problème se produit, car Store Commerce ne peut pas effectuer d’appels réseau vers le même ordinateur (c’est-à-dire s’appeler lui-même). Bien que cela ne devrait jamais être un scénario dans un environnement de production, le problème peut être atténué en activant une exception de bouclage AppContainer afin que les communications puissent se produire sur le même ordinateur. Diverses applications sont accessibles au public pour aider à activer cet envoi en boucle. Pour plus d’informations sur l’envoi en boucle, voir [Activer l’envoi en boucle et résoudre les problèmes d’isolation du réseau](/previous-versions/windows/apps/hh780593(v=win.10)). Il est important de comprendre qu’un envoi en boucle peut constituer un risque pour la sécurité, il n’est donc pas recommandé d’utiliser un envoi en boucle à moins que cela ne soit absolument nécessaire.
