...
5. Effectuez toutes les tâches de configuration requises décrites dans [Configurer Commerce pour l’Autriche](emea-aut-fi-sample.md#set-up-commerce-for-austria).

## <a name="design-of-extensions"></a>Conception des extensions

L’échantillon d’intégration du service d’enregistrement fiscal pour l’Autriche est basé sur la [fonctionnalité d’intégration fiscale](fiscal-integration-for-retail-channel.md). Pour plus d’informations sur la conception de la solution d’intégration fiscale, voir [Vue d’ensemble de l’exemple de conception d’intégration fiscale](fiscal-integration-for-retail-channel.md#fiscal-registration-process-and-fiscal-integration-samples-for-fiscal-devices).
...
Les fichiers de configuration se trouvent dans le dossier **Configuration** du projet d’extension :

- **DocumentProviderFiscalEFRSampleAustria** – Pour les documents fiscaux.
- **DocumentProviderNonFiscalEFRSampleAustria** – Pour les documents non fiscaux.

Ces fichiers ont pour but de permettre le paramétrage du fournisseur pour le fournisseur de documents depuis le siège de Commerce. Le format de fichier est aligné sur les exigences de configuration de l’intégration fiscale. Le paramètre suivant est ajouté :
