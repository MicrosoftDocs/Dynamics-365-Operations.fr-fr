---
title: Relevés de vente au détail
description: Cet article décrit la procédure de création et de validation des relevés.
author: ashishmsft
ms.date: 04/04/2017
ms.topic: article
ms.prod: ''
...
Dans cette étape, les lignes de transaction sont sélectionnées en fonction des critères définis pour chaque magasin dans les pages **Paramètres de commerce** et **Magasins**. Sur ces pages, vous définissez les critères et indiquez comment les transactions sont calculées. Pour afficher la liste des transactions incluses dans le relevé avant de calculer le relevé, utilisez la page **Transactions**.
...
Lorsque vous validez un relevé, les commandes client et les factures sont créées pour les ventes du relevé.
