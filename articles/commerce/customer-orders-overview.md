...
Cette rubrique fournit des informations sur les commandes client dans Modern POS (MPOS). Les commandes client sont également appelées commandes spéciales. La rubrique inclut une discussion sur les paramètres associés et les flux de transaction.
...
Voici quelques paramètres qui peuvent être définis dans la page **Paramètres Commerce** pour définir la façon dont les commandes client sont honorées :
...
### <a name="create-a-customer-order-in-modern-pos"></a>Créer une commande client dans Modern POS
...
1. Cliquez à **Commerce et vente au détail** &gt; **Paramétrage du canal** &gt; **Paramétrage du PDV** &gt; **Profil du PDV** &gt; **Profils de fonctionnalité**.
2. Dans l'organisateur **Général**, définissez l'option **Créer une commande client en mode asynchrone** sur **Oui**.

Lorsque l'option **Créer une commande client en mode asynchrone** est définie sur **Oui**, les commandes client sont toujours créées en mode asynchrone, même si Retail Transaction Service (RTS) est disponible. Si vous définissez cette option sur **Non**, les commandes client sont toujours créées en mode synchrone à l'aide de RTS. Lorsque les commandes client sont créées en mode asynchrone, elles sont extraites et insérées dans les tâches de traction Commerce (P). Les commandes client correspondantes sont créées lorsque l'option **Synchroniser les commandes** est exécutée manuellement ou via un processus de traitement par lots.
