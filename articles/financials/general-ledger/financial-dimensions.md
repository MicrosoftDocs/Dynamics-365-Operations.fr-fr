...
Vous pouvez utiliser les valeurs des enregistrements principaux, tels que les enregistrements client et fournisseur, comme valeurs par défaut dans de nouvelles dimensions. Lorsque les nouvelles dimensions sont créées, l'ID enregistrement principal est entré dans les valeurs de dimension pour ces enregistrements principaux. Par exemple, lorsque vous créez un nouveau client, l'ID client est entré dans la dimension client. Lorsque vous créez des commandes client, des factures ou d'autres documents nécessitant un ID client, les règles par défaut existantes sont utilisées, et l'ID client est ajouté au document.
...
Vous pouvez configurer une dimension afin que les informations des autres dimensions soient automatiquement saisies lorsque vous entrez cette dimension dans un document. Par exemple, si vous entrez le centre de coût 10, la valeur **20** peut être automatiquement entrée dans la dimension de département.
