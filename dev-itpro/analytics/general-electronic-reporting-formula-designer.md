...
La figure suivante présente la création d'une expression de ce type. Dans cet exemple, l'expression renvoie la valeur **Intrastat.AmountMST** du champ de la table **Déclaration d'échanges de biens** de Dynamics 365 for Operations, une fois que la valeur a été arrondie à deux décimales. [image-expression-liaison d'![] (. /media/picture-expression-binding.jpg)](. /media/picture-expression-binding.jpg) Les illustration ci-dessous indique comment une expression de ce type peut être utilisée. Dans cet exemple, le résultat de l'expression conçue est renseigné dans ** Transaction.InvoicedAmount ** le composant ** modèle de déclaration de taxe ** du modèle de données. ![picture-expression-binding2 [] (. /media/picture-expression-binding2.jpg)](. /media/picture-expression-binding2.jpg) Au moment de l'exécution, la formule conçue, ** RONDE (Intrastat.AmountMST, 2) **, arrondira la valeur ** AmountMST ** du champ de chaque enregistrement ** déclaration d'échanges de biens ** de la table à deux décimales, et remplit valeur arrondie ** Transaction.InvoicedAmount ** au composant ** déclaration de taxe ** du modèle de données.

### <a name="data-formatting"></a>Mise en forme des données

Le concepteur de formule de génération d'états électroniques permet de définir une expression qui formate les données reçues des sources de données, afin que les données puissent être envoyées dans le cadre de la génération d'un document électronique. Si vous disposez d'une mise en forme qui doit être appliquée comme règle classique pour être réutilisée pour un format, vous pouvez entrer cette mise en forme une fois dans la configuration du format comme transformation nommée ayant une expression de mise en forme. Cette transformation nommée peut ensuite être liée à de nombreux composants dont la sortie doit être mise en forme en fonction de l'expression créée. La figure suivante présente la création d'une transformation de ce type. Dans cet exemple, la transformation **TrimmedString** prend les données entrantes du type de données **Chaîne**, et tronque les espaces de début et de fin lors du renvoi de la valeur de chaîne. [image-transformation- conception de![(]. /media/picture-transformation-design.jpg)](. /media/picture-transformation-design.jpg) Les illustration ci-dessous indique comment une transformation de ce type peut être utilisée. Dans cet exemple, plusieurs composants de format qui envoient le texte comme résultat de la génération d'un document électronique au moment de l'exécution font référence à la transformation **TrimmedString** par son nom. [image-transformation- utilisation![(]. /media/picture-transformation-usage.jpg)](. /media/picture-transformation-usage.jpg) Lorsque les composants de format se réfèrent ** TrimmedString ** à la transformation (par exemple, ** partyName ** le composant dans l'illustration précédente) qu'il renvoie le texte comme sortie au document générant. Le texte n'inclut pas les espaces de début et de fin. Si vous avez une mise en forme qui doit être appliquée individuellement, vous pouvez introduire cette mise en page en tant qu'expression individuelle d'une liaison d'un composant de format spécifique. La figure suivante présente une expression de ce type. Dans cet exemple, le composant de format **partyType** est lié à la source de données via une expression qui convertit les données entrante du champ **Model.Company.RegistrationType** dans la source de données en texte en majuscules qu'elle envoie comme résultat dans le document électronique. [image-liaison-avec-formule d'![] (. /media/picture-binding-with-formula.jpg)](. /media/picture-binding-with-formula.jpg)
...
[image-contrôle d'![] (. /media/picture-validation.jpg)](. /media/picture-validation.jpg) Concepteur de formule d'ER permet également de spécifier un nom de fichier pour un document électronique la génération et pour contrôler le processus de création du fichier. La figure suivante présente la création d'un contrôle des flux de processus de ce type. Voici une explication de la configuration dans cet exemple :
...
[image-fichier-contrôle d'![] (. /media/picture-file-control.jpg)](. /media/picture-file-control.jpg)
...
Vous pouvez utiliser le texte et les constantes numériques (valeurs qui ne sont pas calculées) lorsque vous concevez des expressions. Par exemple, l'expression ** VALEUR (« 100 ») + 20 ** utilise la constante numérique 20 et la constante » 100 » de chaîne, et retours la valeur numérique ** 120 **. La formule de génération d'états électroniques prend en charge les séquences d'échappement, ce qui signifie que vous pouvez spécifier cette partie de l'expression qui doit être traitée différemment. Par exemple, l'expression **"Léon Tolstoï ""Guerre et paix"" Volume 1"** renvoie la chaîne de texte **Léon Tolstoï "Guerre et paix" Volume 1**.
...
Les opérateurs sur la même ligne ont une priorité identique. Si une expression inclut plusieurs de ces opérateurs, l'expression est évaluée de gauche à droite. Par exemple, l'expression ** \* 1 + 6/2 3 &gt; 5 ** retours ** rectifiez **. Il est recommandé d'utiliser des parenthèses pour indiquer explicitement l'ordre d'évaluation des expressions, afin de faciliter la lecture et la mise à jour des expressions.

#### <a name="references"></a>Références

Toutes les sources de données du composant de génération d'états électroniques actuel (modèle ou format) qui sont disponible lors de la création d'une expression peuvent servir de références nommées. Par exemple, le modèle de données de génération d'états électroniques actuel contient la source de données **ReportingDate** qui renvoie une valeur du type de données **DATETIME**. Pour mettre en forme correctement cette valeur du document générant, vous pouvez référencer la source de données dans l'expression comme suit : ** DATETIMEFORMAT (ReportingDate, « densité- mm - yyyy ») ** tous les caractères {{au:in_the_name_of}} d'{{nom:in_the_name_of}} une source de données référençante qui ne correspondent pas de lettre de l'alphabet doit être précédée marque une simple de devis ('). Si le nom d'une source de données référençante contient au moins un symbole qui ne représente pas une lettre de l'alphabet (par exemple, les signes de ponctuation ou d'autres symboles écrits), le nom doit être guillemets simples dans. Voici quelques exemples :
...
<td><strong>FRACTIONNEMENT (&quot;abcd&quot;, 3)</strong> retourne une nouvelle liste comportant deux enregistrements dont <strong>CHAÎNE</strong> le champ. Le champ dans le premier enregistrement contient le texte <strong>&quot;ABC&quot;</strong>, et le champ dans le deuxième enregistrement contient le texte <strong>&quot;d&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td>Dans l'exemple suivant, la source de données <strong>Lignes</strong> est créée sous la forme d'une liste de trois enregistrements, qui est divisée en traitements par lots, dont chacun contient jusqu'à deux enregistrements. <a href="./media/picture-splitlist-datasource.jpg"><img src="./media/picture-splitlist-datasource.jpg" alt="Data source that is divided into batches" class="alignnone wp-image-290681 size-full" width="397" height="136" /></a> ce champ indique la mise en page conçue de format, où les liaisons <strong>Lignes</strong> à la source de données sont créées pour générer la sortie au format XML qui affiche divers nœuds pour chaque lot et les enregistrements dans celui-ci. <a href="./media/picture-splitlist-format.jpg"><img src="./media/picture-splitlist-format.jpg" alt="Format layout that has bindings to a data source" class="alignnone wp-image-290691 size-full" width="374" height="161" /></a> le suivant est le résultat d'exécuter le format conçu. <a href="./media/picture-splitlist-result.jpg"><img src="./media/picture-splitlist-result.jpg" alt="Result of running the format" class="alignnone wp-image-290701 size-full" width="358" height="191" /></a></td>
</tr>
<tr class="odd">
...
<td><strong>LISTJOIN (FRACTIONNEMENT (&quot;ABC&quot;, 1), FRACTIONNEMENT (&quot;définition&quot;, 1))</strong> renvoie la liste des six enregistrements, où un champ <strong>CHAÎNE</strong> du type de données contient les lettres simples.</td>
</tr>
<tr class="odd">
<td>ISEMPTY (list)</td>
...
<td><strong>EMPTYLIST (FRACTIONNEMENT (&quot;ABC&quot;, 1))</strong> retourne un nouveau vides la liste ayant la même structure à la liste qui est renvoyée par <strong>FRACTIONNEMENT</strong> fonction.</td>
</tr>
<tr class="odd">
<td>FIRST (list)</td>
...
<td>Si vous entrez <strong>FRACTIONNEMENT (&quot;abcdef&quot;, 2)</strong> comme source de données (DS), <strong>NOMBRE (ALLITEMS (DS.Value))</strong> les retours <strong>3</strong>.</td>
</tr>
<tr class="odd">
...
<td>Si <strong>Fournisseur</strong> est configuré comme source de données d'ER qui fait référence à la table VendTable, <strong>LÀ OÙ (fournisseurs, Vendors.VendGroup = &quot;40&quot;)</strong> renvoie la liste de fournisseurs appartenant au groupe de fournisseurs 40.</td>
</tr>
<tr class="even">
...
<td>Dans l'exemple suivant, la source de données <strong>Énumérée</strong> est créée sous la forme d'une liste énumérée des enregistrements fournisseurs de la source de données <strong>Fournisseurs</strong> qui fait référence à la table <strong>VendTable</strong>. <a href="./media/picture-enumerate-datasource.jpg"><img src="./media/picture-enumerate-datasource.jpg" alt="Enumerated data source" class="alignnone wp-image-290711 size-full" width="387" height="136" /></a> est le format, les liaisons de données sont créées pour générer la sortie au format XML qui présente différents fournisseurs comme nœuds répertoriés. <a href="./media/picture-enumerate-format.jpg"><img src="./media/picture-enumerate-format.jpg" alt="Format that has data bindings" class="alignnone wp-image-290721 size-full" width="414" height="138" /></a> c'est le résultat d'exécuter le format conçu. <a href="./media/picture-enumerate-result.jpg"><img src="./media/picture-enumerate-result.jpg" alt="Result of running the format" class="alignnone wp-image-290731 size-full" width="567" height="176" /></a></td>
</tr>
<tr class="odd">
<td>COUNT (list)</td>
...
<td><strong>NOMBRE (FRACTIONNEMENT (&quot;abcd&quot;, 3))</strong> revient <strong>2</strong>, car <strong>FRACTIONNEMENT</strong> la fonction permet de créer une liste comportant deux enregistrements.</td>
</tr>
<tr class="even">
...
<td>L'exemple suivant montre que l'énumération présentée dans un modèle de données. <a href="./media/ger-listoffields-function-model-enumeration.png"><img src="./media/ger-listoffields-function-model-enumeration-e1474545790761.png" alt="GER LISTOFFIELDS function - model enumeration" class="alignnone wp-image-1203943 size-full" width="514" height="155" /></a>les exemple suivant montre :
<ul>
...
<a href="./media/ger-listoffields-function-in-format-expression.png"><img src="./media/ger-listoffields-function-in-format-expression-e1474546110395.png" alt="GER LISTOFFIELDS function - in format expression" class="alignnone wp-image-1204033 size-full" width="549" height="318" /></a> les exemple suivant montre les éléments de format d'ER liés à la source de données de type de liste d'enregistrement créé à l'aide de la fonctionnalité de LISTOFFIELDS.<a href="./media/ger-listoffields-function-format-design.png"><img src="./media/ger-listoffields-function-format-design.png" alt="GER LISTOFFIELDS function - format design" class="alignnone size-full wp-image-1204043" width="466" height="221" /></a>c'est le résultat de l'exécution conçue pour le format. le texte<strong>Remarque :</strong> traduit<a href="./media/ger-listoffields-function-format-output.png"><img src="./media/ger-listoffields-function-format-output.png" alt="GER LISTOFFIELDS function - format output" class="alignnone size-full wp-image-1204053" width="585" height="158" /></a><a href="./media/ger-listoffields-function-format-output.png"><img src="./media/ger-listoffields-function-format-output.png" alt="GER LISTOFFIELDS function - format output" class="alignnone size-full wp-image-1204053" width="585" height="158" /></a>pour les étiquettes et des descriptions est renseigné à la sortie de format d'ER selon les paramètres de langue configurés des éléments pour format parent de FICHIER et de DOSSIER.</td>
</tr>
<tr class="odd">
...
<td>L'exemple suivant illustre le format utilisant des sources de données. <a href="./media/ger-splitlistbylimit-format.png"><img src="./media/ger-splitlistbylimit-format.png" alt="GER SPLITLISTBYLIMIT - format" class="alignnone size-full wp-image-1204063" width="396" height="195" /></a><a href="./media/ger-splitlistbylimit-datasources.png"><img src="./media/ger-splitlistbylimit-datasources.png" alt="GER SPLITLISTBYLIMIT - datasources" class="alignnone size-full wp-image-1204073" width="320" height="208" /></a>c'est l'exécution du format de résultat qui affiche la liste plate des articles de la marchandise.<a href="./media/ger-splitlistbylimit-output.png"><img src="./media/ger-splitlistbylimit-output.png" alt="GER SPLITLISTBYLIMIT - output" class="alignnone size-full wp-image-1204083" width="462" height="204" /></a>les exemple suivant montre le même format ajusté pour afficher la liste des articles de marchandise par lots lorsqu'un seul lot doit inclure des marchandises avec le poids total qui ne doit pas dépasser la limite de 9.<a href="./media/ger-splitlistbylimit-format-1.png"><img src="./media/ger-splitlistbylimit-format-1.png" alt="GER SPLITLISTBYLIMIT - format 1" class="alignnone size-full wp-image-1204103" width="466" height="438" /></a><a href="./media/ger-splitlistbylimit-datasources-1.png"><img src="./media/ger-splitlistbylimit-datasources-1.png" alt="GER SPLITLISTBYLIMIT - datasources 1" class="alignnone size-full wp-image-1204093" width="645" height="507" /></a>c'est le résultat de l'exécution ajustée de format. <a href="./media/ger-splitlistbylimit-output-1.png"><img src="./media/ger-splitlistbylimit-output-1.png" alt="GER SPLITLISTBYLIMIT - output 1" class="alignnone size-full wp-image-1204113" width="676" height="611" /></a><strong>Remarque :</strong> la limite n'est pas appliqué au dernier élément de la liste d'origine comme valeur (11) de la source de sa limite (poids) dépasse la limite définie (9). Utilisez la fonction <strong>WHERE</strong> ou l'expression <strong>Activé</strong> de l'élément de format correspondant pour ignorer les sous-listes lors de la génération d'états (si nécessaire).</td>
</tr>
<tr class="odd">
...
<td>Le FILTRE (fournisseurs, Vendors.VendGroup = &quot;40&quot;) renvoie la liste des fournisseurs uniquement appartenant au groupe « 40 "fournisseurs lorsque <strong>Fournisseur</strong> est configuré comme source de données d'ER faire <strong>VendTable</strong> référence à la table</td>
</tr>
</tbody>
</table>
...
| OU (condition 1\[, condition 2,…\])                                                  | Renvoie la valeur **FALSE** si *toutes *les conditions spécifiées sont fausses. Renvoie la valeur **TRUE** si *l'une *des conditions spécifiées est vraie.                                                                                                                                                                 | **OR (1=2, "a"="a")** renvoie **TRUE**.                                                                                                                                                                                                                      |
...
<td><strong>NUMBERVALUE (&quot;1 234,56&quot;, &quot;,&quot;, &quot; &quot;)</strong> retourne la valeur <strong>1234,56</strong>.</td>
</tr>
<tr class="even">
<td>VALUE (string)</td>
...
<td><strong>VALEUR (&quot;1 234,56&quot;)</strong> Une exception contrat.</td>
</tr>
<tr class="odd">
...
| EMPTYRECORD (record) | Renvoie un enregistrement **null** ayant la même structure que la liste d'enregistrements ou l'enregistrement spécifié. ** Remarque : ** A ** nulle ** enregistrement est un enregistrement dans lequel tous les champs ont une valeur nulle (** 0 ** \[\] zéro pour les numéros, une chaîne vide pour les chaînes, etc.). | **EMPTYRECORD (SPLIT ("abc", 1))** renvoie un nouvel enregistrement vide ayant la même structure que la liste qui est renvoyée par la fonction **SPLIT**.   |
...
<td><strong>MI (&quot;Exemple&quot;, 2, 3)</strong> redevient <strong>&quot;ampère&quot;</strong>.</td>
</tr>
<tr class="even">
<td>LEN (string)</td>
<td>Retourne le nombre de caractères dans la chaîne spécifiée.</td>
<td><strong>LEN (&quot;Exemple&quot;)</strong> redevient <strong>6</strong>.</td>
</tr>
<tr class="odd">
<td>CHAR (number)</td>
<td>Permet de renvoyer la chaîne de caractères référencée par le nombre Unicode spécifié.</td>
<td><strong>CHAR (255)</strong> retours <strong>&quot;ÿ&quot;</strong>. <strong>Remarque :</strong> la chaîne retournée dépend de l'encodage sélectionné dans l'élément de format FILE parent. La liste d'encodages pris en charge figure dans la rubrique <a href="https://msdn.microsoft.com/en-us/library/system.text.encoding(v=vs.110).aspx">Classe d'encodage</a>.</td>
</tr>
<tr class="even">
<td>CONCATENATE (string 1 [, string 2, …])</td>
<td>Renvoie toutes les chaînes de texte spécifiées, associées dans une chaîne.</td>
<td><strong>CONCATÉNEZ (&quot;ABC&quot;, &quot;définition&quot;)</strong> retours <strong>&quot;abcdef&quot;</strong>. <strong>Remarque :</strong> L'expression <strong>&quot;ABC&quot; &amp; &quot;définition&quot;</strong> les retours également <strong>&quot;abcdef&quot;</strong>.</td>
</tr>
<tr class="odd">
...
<td><strong>TRADUISEZ (&quot;abcdef&quot;, &quot;cd&quot;, &quot;GH&quot;)</strong> remplace le modèle <strong>&quot;cd&quot;</strong> par la chaîne <strong>&quot;GH&quot;</strong> et redevient <strong>&quot;abGHef&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td> <strong>REMPLACEZ (&quot;+1 923.456 4971&quot;, &quot;[] ^0-9&quot;, &quot;&quot;, real)</strong> applique une expression ordinaire qui supprime tous les symboles non-numériques, et redevient <strong>&quot;19234564971&quot;</strong>. <strong>REMPLACEZ (&quot;abcdef&quot;, &quot;cd&quot;, &quot;GH&quot;, ou)</strong> remplace le modèle <strong>&quot;cd&quot;</strong> par la chaîne <strong>&quot;GH&quot;</strong> et redevient <strong>&quot;abGHef&quot;</strong>.</td>
</tr>
<tr class="odd">
<td>TEXT (input)</td>
...
<td>Si Dynamics 365 pour les paramètres régionaux du serveur d'instance d'opérations est défini comme <strong>EN-US</strong>, <strong>TEXTE (MAINTENANT ())</strong> fait Dynamics actuel 365 pour la date de la session d'opérations, 12/17/2015, comme chaînes de <strong>&quot;12/17/2015 7h59 : 23H DU MATIN&quot;</strong>texte. <strong>TEXTE (1/3)</strong> redevient <strong>&quot;0,33&quot;</strong>.</td>
</tr>
<tr class="even">
<td>FORMAT (string 1, string 2[, string 3, ...])</td>
<td>Permet de renvoyer la chaîne spécifiée, qui est mise en forme en substituant toutes les occurrences de <strong>%N</strong> par le <em>n</em>ième argument. Les arguments sont des chaînes. Si un argument n'est pas proposé pour un paramètre, le paramètre est retourné comme <strong>&quot;%N&quot;</strong> dans la chaîne. Pour les valeurs de type <strong>real</strong>, la conversion de chaîne est limitée à deux décimales.</td>
<td>Dans cet exemple, la source de données <strong>PaymentModel</strong> renvoie la liste des enregistrements client via le composant <strong>Customer</strong> et la valeur de la date de traitement via le champ <strong>ProcessingDate</strong>. <a href="./media/picture-format-datasource.jpg"><img src="./media/picture-format-datasource.jpg" alt="PaymentModel data source" class="alignnone wp-image-290751 size-full" width="293" height="143" /></a> Dans le format d'ER qui est conçu pour générer un fichier électronique pour les clients sélectionnés, <strong>PaymentModel</strong> est sélectionné comme source de données et contrôle le processus. Une exception est entraînée pour les utilisateurs finaux lorsqu'un client sélectionné est arrêté à la date lorsque l'état est traité. La formule qui est conçue pour ce type de contrôle de traitement peut utiliser les ressources suivantes :
<ul>
<li>Libellé Dynamics 365 for Operations SYS70894, avec le texte suivant :
<ul>
<li><strong>Pour la langue d'EN-US :</strong> &quot;Rien à imprimer&quot;</li>
<li><strong>Pour DE langage :</strong> &quot;Le zu de Nichts drucken&quot;</li>
</ul></li>
<li>Libellé Dynamics 365 for Operations SYS18389, avec le texte suivant :
<ul>
<li><strong>Pour la langue d'EN-US :</strong> &quot;Le client s'est bloqué pour %2.&quot;</li>
<li><strong>Pour DE langage :</strong> &quot;Débiteur « %1" gesperrt du für %2 de wird.&quot;</li>
</ul></li>
</ul>
Voici la formule qui peut être professionnels : FORMAT (CONCATÉNEZ (@SYS70894&quot;&quot;&quot;. &quot;, @SYS18389&quot;&quot;), Model.Customer.Name, DATETIMEFORMAT (model.ProcessingDate, D)) si un rapport est&quot;exécuté pour le 17 décembre 2015 <strong>Client au détail de Litware</strong>, dans la culture et la langue <strong>EN-US</strong>, des retours de cette <strong>EN-US</strong> formule le texte suivant, qui peut être présentés comme un message d'exception pour l'utilisateur : Rien à imprimer. &quot; La vente au détail de Litware de client est interrompue pour 12/17/2015.&quot; Si le même état est exécuté pour<strong> Client au détail de Litware</strong> le 17 décembre 2015, dans <strong>De</strong> la culture et <strong>De</strong> la langue, des retours de cette formule le texte suivant, utilisant un format de date différente : &quot;Le zu de Nichts drucken. Gesperrt au détail du für 17.12.2015 de wird « Litware de débit ».&quot; <strong>Remarque : </strong>la syntaxe suivante est appliquée aux formules de génération d'états électroniques pour les libellés :
<ul>
<li><strong>Pour les étiquettes de Dynamics 365 pour les ressources opérationnelles :</strong><strong>@&quot;O&quot;</strong>, où X est l'ID libellé dans l'arbre d'objets d'application (AOT)</li>
<li><strong>Pour les étiquettes résidant dans des configurations d'ER :</strong><strong>@&quot;ER_LABEL : O&quot;</strong>, où X est l'ID libellé dans la configuration d'ER</li>
</ul></td>
</tr>
<tr class="odd">
<td>NUMBERFORMAT (number, format)</td>
<td>Renvoie la représentation sous forme de chaîne du nombre spécifié au format spécifié. (Pour plus d'informations sur les formats pris en charge, voir <a href="https://msdn.microsoft.com/en-us/library/dwhawy9k(v=vs.110).aspx">standard</a> et <a href="https://msdn.microsoft.com/en-us/library/0c899ak8(v=vs.110).aspx">personnalisés</a>.) Le contexte que cette fonction est exécutée dans détermine la culture utilisée pour déterminer le format des numéros.</td>
<td>Pour la culture d'EN-US, <strong>NUMBERFORMAT (0,45, &quot;p&quot;)</strong> retours <strong>&quot;45.00 %&quot;</strong>. <strong>NUMBERFORMAT (10,45, &quot;#&quot;)</strong> redevient <strong>&quot;10&quot;</strong>.</td>
</tr>
<tr class="even">
...
<td>NUMERALSTOTEXT (1234,56, &quot;en&quot;, fausse, 2) renvoie « kilomètre deux centimes trente-quatre et 56 "NUMERALSTOTEXT (120, &quot;PL&quot;, ou, 0) renvoie dwadzieścia » NUMERALSTOTEXT « de Sto (120,21, &quot;RU&quot;, &quot;EUR&quot;, real, 2) renvoie le « евроцент de Сто двадцать евро 21 »</td>
</tr>
<tr class="odd">
...
SUMIFS (chaîne clés pour additionner, Chaîne de critères range1, \[chaîne de valeur 1 de critères, Chaîne de critères range2, Chaîne de valeur 2 de critères,…\])
...
COUNTIFS (chaîne de critères range1, \[chaîne de valeur 1 de critères, Chaîne de critères range2, Chaîne de valeur 2 de critères,…\])
...
COLLECTEDLIST (chaîne de critères range1, \[chaîne de valeur 1 de critères, Chaîne de critères range2, Chaîne de valeur 2 de critères,…\])
...
| Modèle\_97 (chiffres)                                                                 | Renvoie une référence créditeur, comme une expression MOD97, selon les chiffres du numéro de facture spécifié.                                                                                                                                                            | ** Modèle\_97 (« VEND-200002 ») ** retours ** « 20000285 " **.                                                                                                                                                                                                                                                           |
...
| NC\_GBT\_AdditionalDimensionID (chaîne, numéro)                                  | Obtenir l'ID de dimension financière supplémentaire. Les dimensions sont représentées dans cette chaîne comme des ID séparés par des virgules. Les numéros définissent le code souche de la dimension demandée dans cette chaîne.                                                                            | NC\_GBT\_AdditionalDimensionID (« CA, BB, cc, densité double, EE, FF, GG, HH », 3) retour « cc »                                                                                                                                                                                                                                      |
| GetCurrentCompany ()                                                             | Renvoie le code de la société enregistrée actuelle.                                                                                                                                                                                                                    |                                                                                                                                                                                                                                                                                                               |
| Modèle\_10 (chiffres) de\_de BANK de ch\_                                                       | Renvoie une référence créditeur, comme une expression MOD10, selon les chiffres du numéro de facture spécifié.                                                                                                                                                                      | Modèle\_\_\_BANK de ch\_10 (« VEND-200002 ») retours 3                                                                                                                                                                                                                                                                   |
| SOMME de Immobilisations\_(code d'immobilisation, code modèle de valeur, date de début, Date de fin)               | Retourne le conteneur de données préparé des montants d'une immobilisation pour une période.                                                                                                                                                                                         | La SOMME de Immobilisations\_(« COMP-000001 », « en », Date1, Date2) renvoie le conteneur préparé de données de l'immobilisation « COMP-000001 » avec le modèle de valeur « en » pour une période de Date1 à Date2.                                                                                                                        |
| SOLDE de Immobilisations\_(code d'immobilisation, code modèle de valeur, d'organigramme d'entreprise l'année, d'organigramme d'entreprise la date) | Retourne le conteneur de données préparé des soldes d'une immobilisation. L'année de génération d'états doit être spécifiée comme valeur de l'énumération Dynamics 365 for Operations **AssetYear**.                                                                                           | SOMME de Immobilisations\_(« COMP-000001 « , « en », année d'AxEnumAssetYear.This, SESSIONTODAY ()) renvoie le conteneur préparé de données de soldes pour l'immobilisation « COMP-000001 » avec le modèle de valeur « en » sur le actuel 365 pour la date de la session d'opérations.                                                                |
