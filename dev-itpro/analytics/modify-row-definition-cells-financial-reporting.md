...
-   le libellé doit commencer par une lettre (a à z ou A à Z), et peut être n'importe quelle combinaison des chiffres et de lettres jusqu'à 16 caractères. ** Remarque : ** Une étiquette peut inclure le trait de soulignement (\_), mais pas d'autres caractères spéciaux.
...
3.  Sélectionnez un code de format dans la liste. Le tableau suivant décrit les codes de format et leurs actions.
    ...
    | TOT                           | Total                              | Identifie une ligne qui utilise des opérateurs mathématiques dans la colonne **Formules/Lignes/Unités associées**. Les totaux contiennent des opérateurs simples, par exemple **+** ou **-**.                                                      |
    | CAL                           | Calcul                        | Identifie une ligne qui utilise des opérateurs mathématiques dans la colonne **Formules/Lignes/Unités associées**. Les calculs contiennent des opérateurs complexes, par exemple **+**, **-**, **\***, **/**, et ** IF/THEN/ELSE ** des relevés. |
...
Dans l'exemple suivant, Phyllis souhaite empêcher que le titre et les traits de soulignement dans la ligne **Disponibilités totales** de son état soient imprimés, car il n'y a eu aucune activité dans aucun des comptes de disponibilités. Par conséquent, de la ligne 220 (qui, comme **---** code format indique, est une ligne de mise en forme), dans ** de Formules/Lignes/Unités connexes ** la cellule, elle entre ** 250 **, qui est un code ligne du montant de ligne qu'elle souhaite supprimer. ![RelatedRowsRowDefinition [] (. /media/relatedrowsrowdefinition-1024x144.png)](. /media/relatedrowsrowdefinition.png)
...
6.  Dans le menu **Fichier**, cliquez sur **Enregistrer sous** pour enregistrer la définition de colonne sous un nouveau nom. Permet d'ajouter ** MLB ** au nom de fichier actuel (par exemple, ** CABOT\_YTD\_MLB **). Cette définition de colonne est votre définition de colonne de base.
...
Dans l'exemple suivant d'une définition de ligne, la ligne 100 indique que la ligne de base des calculs est la ligne 280. [exemple de![de calcul de ligne de base.] (. /media/cbrrowdefinition.png)](. /media/cbrrowdefinition.png) Dans l'exemple suivant d'une définition de colonne, les calculs utilisent le code de format **CBR**. Le calcul dans la colonne C divise la valeur de la colonne B de l'état par la valeur contenue dans la ligne 280 et la colonne B. Le remplacement de format de la colonne B imprime le résultat du calcul sous la forme d'un pourcentage. De même, chaque montant dans la colonne E correspond au montant dans la colonne D sous la forme d'un pourcentage des ventes nettes. [exemple de définition de colonne d'![.] (. /media/cbrcolumndefinition2.png)](. /media/cbrcolumndefinition2.png) L'exemple suivant présente un état pouvant être généré à partir des calculs précédents. [calculs de l'exemple précédent de l'état des exemples![sur.] (. /media/cbrreport-1024x272.png)](. /media/cbrreport.png)
...
Dans l'exemple suivant, les valeurs de la colonne D de l'état sont triées dans l'ordre croissant pour les lignes 160 à 490. En outre, les valeurs absolues de la colonne G de l'état sont triées dans l'ordre décroissant pour les lignes 610 à 940.
...
5.  Sous **Attributs**, procédez comme suit pour ajouter une entrée pour chaque attribut devant être inclus avec le code de ligne :
    1.  Double-cliquez sur la cellule **Attribut**, puis sélectionnez un nom d'attribut. ** Attention : ** Remplacez le signe dièse (\#) par une valeur numérique.
...
5.  Dans ** origine ** le champ, entrez la dimension de début, ou cliquez sur ! Accédez [] (https://i-technet.sec.s-msft.com/dynimg/IC679490.gif « accèdent ») à la recherche des dimensions disponibles. Pour entrer une plage de dimensions, entrez la dimension de fin dans le champ **Jusqu'à**.
...
À quel que vous entrez-vous une valeur de segment naturel dans ** des dimensions ** la boîte de dialogue, vous pouvez placer un caractère générique (? ou \*) à n'importe quelle position d'un segment. Le Concepteur de rapports extrait toutes les valeurs pour les positions définies sans tenir compte des caractères génériques. Par exemple, la définition de ligne ne contient que des valeur de segment naturel, et les segments naturels ont quatre caractères. En entrant ** 6 ? ? ? ** dans une ligne, vous indiquez à Concepteur de rapports inclure tous les comptes avec une valeur de segment naturel commençant par les 6. Si vous entrez 6 **\***, les mêmes résultats s'affichent, mais résulte incluent également les valeurs de variable - largeur, par exemple ** 60 ** et ** 600000 **. Le générateur d'états remplace chaque caractère générique (?) par la gamme complète des valeurs possibles, qui incluent des lettres et des caractères spéciaux. Par exemple, dans la plage **12?0** à **12?4**, le caractère générique dans **12?0** est remplacé par la valeur la plus faible dans le jeu de caractères, et le caractère générique dans **12?4** est remplacé par la valeur la plus élevée dans le jeu de caractères. **Remarque :** vous devez éviter d'utiliser des caractères génériques pour les comptes de début et de fin de plages. Si vous utilisez des caractères génériques dans le compte de début ou le compte de fin, vous pouvez obtenir des résultats inattendus.

### <a name="single-segment-or-single-dimension-ranges"></a>Plages comprenant un seul segment ou une seule dimension

Vous pouvez spécifier une plage de valeurs de segment ou de valeurs de dimension. L'avantage de spécifier une plage est que vous n'avez pas besoin mettre la définition de ligne à jour à chaque fois qu'une nouvelle valeur de segment ou de dimension est ajoutée aux données financières. Par exemple, la plage ** 6100:6900\]+Compte=\[** tractions les valeurs des comptes 6100 à 6900 dans le montant de la ligne. Lorsqu'une plage inclut un caractère générique (?), le générateur d'états n'évalue pas la plage caractère par caractères. Au lieu de cela, on détermine le haut et le bas de la plage, puis les valeurs extrêmes dans cette plage et toutes les valeurs entre elles. ** Remarque : ** Le Concepteur de rapports ne peut pas sélectionner les comptes, les dimensions, les champs ou du système Microsoft Dynamics ERP incluant les caractères réservés suivants : et, \*, \[, \], {, ou}. Vous pouvez ajouter une esperluette (&) que lorsque vous générez automatiquement des définitions de ligne à l'aide de la boîte de dialogue **Insérer des lignes à partir de dimensions** .

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Plages de plusieurs segment ou de plusieurs dimensions

Lorsque vous entrez une plage à l'aide de les combinaisons de plusieurs valeurs de dimension, la comparaison de la plage est effectuée sur l'A.\\ dimensions financières base de dimension par dimension. La comparaison de plage ne peut pas être effectuée caractère par caractère ou sur la base d'un segment partiel. Par exemple, la plage ** 5000:6000\]1000:2000,\], center=\[00\]+Compte=\[Service=\[de coût ** inclut que les comptes qui correspondent à chaque segment. Dans ce scénario, le premier segment doit se trouver dans la plage comprise entre 5000 et 6000, le second segment doit se trouver dans la plage de 1000 le jusqu'en 2000, et le dernier segment doit être 00. Par exemple, ** +Compte=\[5100\], Service=\[1100\], center=\[01\]de coût ** n'est pas inclus dans l'état, car le dernier segment dehors de la plage spécifiée. Si une valeur de segment inclut des espaces, placez cette valeur entre crochets (\[ \]). Les valeurs suivantes sont valides pour un segment à quatre caractères : **\[ 234\], \[123 \], \[1 34\]**. Les valeurs de dimension doivent être placées entre crochets (\[ \]), et le Concepteur de rapports ajoute les parenthèses pour vous. À quel moment une plage contenant plusieurs segments ou dimensions inclut des caractères génériques (? ou \*), les valeurs de début et de fin de la plage complète de contenant plusieurs segments ou dimensions sont déterminées, puis les valeurs de fin et toutes les valeurs entre eux sont incluses. Si vous avez une plage étendue, comme toute la plage de comptes de 40000 à 99999, vous devez spécifier un compte de début et un compte de fin valides à chaque fois que c'est possible. ** Remarque : ** Le Concepteur de rapports ne peut pas sélectionner les comptes, les dimensions, les champs ou du système Microsoft Dynamics ERP incluant les caractères réservés suivants : et, \*, \[, \], {, ou}. Vous pouvez ajouter une esperluette (&) que lorsque vous générez automatiquement des définitions de ligne à l'aide de la boîte de dialogue **Insérer des lignes à partir de dimensions** .
...
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| Additionner deux comptes pleinement qualifiés.                                                       | +Division=\[000\], Compte=\[1205\], Service=\[00\]+Division=\[100\], Compte=\[1205\], Service=\[00\] |
...
| Additionner une plage de comptes pleinement qualifiés.                                                | 000:100\], Compte=\[1205\], Service=\[00\]+Division=\[                                                   |
| Additionner une plage de valeurs de segment.                                                          | 1200:1205\]+Compte=\[                                                                                       |
| Additionner une plage de valeurs de segment qui incluent des caractères génériques.                         | +Compte=\[120 ? : 130 ?\]                                                                                       |
| Soustraire un compte pleinement qualifié d'un autre compte pleinement qualifié.              | +Division=\[000\], Compte=\[1205\], Service=\[00\]- Division=\[100\], Compte=\[1205\], Service=\[00\] |
...
| Soustraire une plage de comptes pleinement qualifiés.                                           | - 000:100\]1200:1205,\], 0h01\]de Division=\[de Compte=\[Service=\[                                           |
| Soustraire une plage de valeurs de segment.                                                     | - 1200:1205\]de Compte=\[                                                                                       |
| Soustraire une plage de valeurs de segment qui incluent des caractères génériques.                    | - Compte=\[120 ? : 130 ?\]                                                                                       |

Bien que vous puissiez modifier les comptes directement, vous pouvez également utiliser la boîte de dialogue **Dimensions** pour appliquer la mise en forme correcte à vos liaisons aux données financières. Les valeurs peuvent inclure des caractères génériques (? ou \*). Toutefois, le Concepteur de rapports ne peut pas sélectionner les comptes, les dimensions, les champs ou du système Microsoft Dynamics ERP incluant les caractères réservés suivants : et, \*, \[, \], {, ou}. **Remarque :** pour soustraire des valeurs, vous devez mettre des parenthèses autour de ces valeurs. Par exemple, si vous entrez 450 ? **- (4509) **, il affiché comme ** +Compte=\[4509\]- Compte=\[450 ?\]**, et vous instruisent le Concepteur de rapports soustraire le montant du segment de compte 4509 du montant des segments de compte commençant par 450.
...
3.  Dans la première ligne de la boîte de dialogue **Dimensions**, procédez comme suit :
    ...
    2.  Double-cliquez sur ** opérateur +/- ** la cellule, puis sélectionnez le signe plus (**+**) ou (**-**) l'opérateur négatif qui s'applique à une ou plusieurs valeurs de dimension ou ensembles dans la ligne.
...
| Article                | Description                                                                                                                                                                                                                                                                                             |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Individuelle ou plage | Dans ** origine ** le champ, entrez le nom d'un compte, ou cliquez sur ** accédez ** le bouton ! Accédez [] (https://i-technet.sec.s-msft.com/dynimg/IC679490.gif « accèdent ») pour rechercher le compte. Pour sélectionner une plage, entrez ou cherchez une valeur dans le champ **Jusqu'à**.                                             |
| Ensemble de valeurs de dimension | Dans le champ **Nom**, entrez le nom d'un ensemble de valeurs de dimensions. Pour créer, modifier, copier ou supprimer un ensemble, cliquez sur **Gérer les ensembles de valeurs de dimension**. ** Formule ** le champ est complété à l'aide de la formule ** de lier aux dimensions financières ** de la cellule pour cette valeur de dimension définie dans la définition de ligne. |
