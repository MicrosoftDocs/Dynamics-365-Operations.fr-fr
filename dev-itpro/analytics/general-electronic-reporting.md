...
-   il représente un outil commun unique pour la génération d'états électronique dans différents domaines et remplace plus de 20 moteurs différents effectuant certains types de génération d'états électronique Microsoft Dynamics 365 for Operations.
...
-   Il prend en charge la transformation des données de Dynamics 365 for Operations dans les groupes requis, le filtrage, le tri et l'addition des données et également l'ajout avec les champs logiquement calculés qui sont conçus par des formules Microsoft Excel (pour plus d’informations, consultez [Concepteur de formule dans les états électroniques](general-electronic-reporting-formula-designer.md)).

[![Éditeur de formules de type Excel](./media/pic-formula-1024x615.png)](./media/pic-formula.png) Un composant de modèle est conçu pour chaque domaine d'entreprise afin d'être utilisé en tant que source de données unifiée pour la génération d'états qui isole la génération d'états de la mise en œuvre physique des sources de données Microsoft Dynamics 365 for Operations, représente des concepts et des fonctionnalités d'affaires spécifiques au domaine sous une forme qui rend la conception initiale et la mise à jour des formats de génération d'états plus efficaces.
...
-   La version **TERMINÉE** peut être convertie en une version **PARTAGÉE**. Cette version est publiée sur LCS et peut être utilisée dans le processus de création de rapports globaux.
-   La version **PARTAGÉE** peut être convertie en une version **INTERROMPUE**. Cette version peut ensuite être supprimée.

Les versions dotées du statut** TERMINÉE** ou **PARTAGÉE** sont disponibles pour un autre échange de données. Un composant possédant ces statuts peut avoir ces actions effectuées sur ceux-ci :

-   Ils peuvent être sérialisés au format XML et exportés vers un fichier au format XML à partir de Dynamics 365 for Operations.
-   Ils peuvent être resérialisés à partir d’un fichier XML et importés dans Dynamics 365 for Operations sous la forme d’une nouvelle version d’un composant d'état électronique.
...
L'accès aux composants de format d'états électroniques dépend du paramètre du code de pays/région ISO. Lorsque ce paramètre est vide pour une version sélectionnée d’une configuration de format, un composant de format est accessible à partir de n’importe quelle société Dynamics 365 for Operations au moment de l’exécution. Lorsqu'il contient des codes pays/région ISO, un composant de format est disponible uniquement à partir des sociétés Dynamics 365 for Operations dont l'adresse principale est défini pour l'un des codes pays/régions ISO du composant de format. Différentes versions d'un composant de format de données peuvent avoir différents paramètres de codes pays/région ISO.
...
Un référentiel d'états électroniques enregistre les configurations d'états électroniques. Les types de référentiels d'états électroniques suivants sont actuellement pris en charge : **ressources opérationnelles** et **projet LCS**. Le référentiel de **ressources opérationnelles** permet d'accéder à la liste des configurations livrées dans le cadre de la solution Dynamics 365 for Operations par Microsoft en tant que fournisseur de configurations d'états électroniques. Ces configurations peuvent être importées dans l’instance actuelle de Dynamics 365 for Operations et utilisées pour l'état électronique. Elles peuvent également être utilisées pour des localisations/personnalisations. Un référentiel de **projet LCS** permet d'accéder à la liste des configurations d'un projet LCS spécifique (bibliothèque d'actifs de projet LCS) sélectionné à l'enregistrement du référentiel. Les états électroniques vous permettent de télécharger des configurations partagées de l'instance Dynamics 365 for Operations actuelle vers un référentiel **Projet LCS** donné. Vous pouvez également importer des configurations depuis un référentiel **Projet LCS** donné vers l'instance Dynamics 365 for Operations actuelle. Les référentiels **Projet LCS** requis peuvent être enregistrés individuellement pour chaque fournisseur de configuration de l'instance Dynamics 365 for Operations actuelle. Chaque référentiel peut être consacré à un fournisseur de configuration spécifique.

## <a name="supported-scenarios"></a>Scénarios pris en charge
### <a name="building-a-data-model"></a>Élaboration d'un modèle de données

L'état électronique propose un concepteur de modèle vous permettant de créer un modèle de données pour un domaine de l'entreprise donné. Toutes les entités commerciales spécifiques à un domaine et les relations entre elles peuvent être présentées dans un modèle de données en tant que structure hiérarchique. L’illustration suivante montre un exemple de ce type de modèle de données (le modèle de données du domaine de paiement). [![Exemple de modèle de données](./media/pic-data-model-1024x550.png)](./media/pic-data-model.png) Pour vous familiariser avec ce scénario en détails, visionnez le guide de tâche **ER Concevoir un modèle de données spécifiques au domaine** (qui fait partie du processus d'entreprise **7.5.4.3 Acquérir/Développer des composants de services/solutions informatiques (10677)**.

### <a name="translating-data-model-content"></a>Traduction du contenu du modèle de données

Le contenu du modèle de données (étiquettes et descriptions) peut être traduit dans d'autres langues prises en charge par Dynamics 365 for Operations. Vous souhaitez peut-être traduire le contenu du modèle de données pour les raisons suivantes :
...
-   au moment de l'exécution, pour rendre le contenu plus convivial pour la présentation des invites et de l'aide pour les paramètres d'exécution ainsi que des messages de validation configurés (erreurs et avertissements) dans la langue que l'utilisateur actuellement connecté à Microsoft Dynamics 365 for Operations préfère.

L’illustration suivante montre un exemple de la traduction du contenu du modèle de données de l’anglais vers le japonais. [![Contenu du modèle de données en anglais](./media/pic-translate-en-1024x495.png)](./media/pic-translate-en.png) [![Contenu du modèle de données traduit en japonais](./media/pic-translate-ja-1024x495.png)](./media/pic-translate-ja.png)

### <a name="configuring-data-model-mappings"></a>Configuration des mises en correspondance de modèles de données

L'état électronique fournit un concepteur de mise en correspondance de modèle qui permet aux utilisateurs de mapper des modèles de données qu'ils ont conçus pour des sources de données Dynamics 365 for Operations spécifiques. L'illustration suivante présente un exemple de ce type de mise en correspondance de modèle de données (mise en correspondance du modèle **Virement SEPA** avec le modèle de données du domaine de paiement). [![Exemple de mise en correspondance de modèle de données ](./media/pic-model-mapping-1024x551.png)](./media/pic-model-mapping.png) Pour vous familiariser avec ce scénario en détails, visionnez les guides de tâche **ER Définir le mappage de modèles et sélectionner des sources de données** et **ER Mapper le modèle de données aux sources de données sélectionnées** (qui font partie du processus d'entreprise **7.5.4.3 Acquérir/Développer des composants de services/solutions informatiques (10677)**).
...
ER peut stocker un modèle de données conçu avec des mises en correspondance de données associées en tant que configuration de modèle de l'instance actuelle Dynamics 365 for Operations. L’illustration suivante montre un exemple de ce type de configuration de modèle de données (la configuration du modèle de paiement). [![Exemple de configuration de modèle de données](./media/pic-model-configuration-1024x585.png)](./media/pic-model-configuration.png) Pour vous familiariser avec ce scénario en détails, visionnez le guide de tâche **ER Concevoir un modèle de données spécifiques au domaine** (qui fait partie du processus d'entreprise **7.5.4.3 Acquérir/Développer des composants de services/solutions informatiques (10677)**.
...
Le concepteur de format ER peut être utilisé pour générer un document électronique au format de feuille de calcul OPENXML. L’illustration suivante montre un exemple de ce type de format (configuration de format permettant de générer une feuille de calcul OPENXML avec les détails d’un journal des paiements sélectionné) :[![Pic-ER-format-Excel](./media/pic-er-format-excel.jpg)](./media/pic-er-format-excel.jpg) Pour vous familiariser avec ce scénario en détails, visionnez le guide de tâche **ER Créer une configuration pour des états au format OPENXML** (qui fait partie du processus d'entreprise **7.5.4.3 Acquérir/Développer des composants de services/solutions informatiques (10677)**). Utilisez le fichier Excel ci-dessous en tant que modèle du format de conception ER pour terminer l’étape de l’importation d’un modèle de format de ce guide de tâche : [Modèle d'état de paiement (SampleVendPaymWsReport.xlsx)](https://go.microsoft.com/fwlink/?linkid=845202)
...
Vous pouvez configurer Dynamics 365 for Operations pour utiliser le format créé pour générer des états électroniques. La référence à la configuration de format créé doit être définie dans les paramètres d'un domaine particulier. Par exemple, pour commencer à utiliser une configuration de format ER pour les paiements fournisseur électroniques au format BACS, la configuration de format doit être référencée dans des modes de paiement spécifiques, comme indiqué dans les illustrations suivantes : 

[![Format de la configuration BACS (Royaume-Uni)](media/ger-bacs-uk-format-configuration.png) 

[![Référencement du format BACS (Royaume-Uni) dans un mode de paiement](media/ger-bacs-uk-format-method.png) 
...
## <a name="list-of-er-configurations-that-are-delivered-in-the-dynamics-365-for-operations-solution"></a>Liste de configurations ER qui sont fournies dans la solution de Dynamics 365 for Operations
