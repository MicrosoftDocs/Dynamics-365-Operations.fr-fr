...
Le fait de classer les modèles permet d'évaluer le niveau de qualification réel d'une personne, le niveau qu'ils doivent atteindre, ou le niveau de la qualification nécessaire pour une tâche. Vous pouvez entrer jusqu'à 10 niveaux pour un modèle de classement.  Chaque niveau dans un modèle de classement est affecté un facteur.  La valeur du facteur est utilisée pour normaliser scores de qualifications qui utilisent différents modèles de classement.  Facteur doit être un nombre compris entre 0-9 et chaque niveau doit disposer d'un facteur.  Les niveaux avec des valeurs de facteur plus élevées ont plus de poids dans un modèle de classement.

## <a name="specify-job-skills"></a>Spécification des qualifications requises pour une tâche
Lorsque vous entrez des informations sur une tâche, vous pouvez spécifier les qualifications qu'une personne si devez effectuer le travail requis pour la tâche.  En outre vous pouvez spécifier le niveau désiré pour chaque qualification également le niveau d'importance de la qualification. Différents postes peuvent nécessiter différents niveaux d'importance d'une même qualification.
...
Vous pouvez créer une recherche de mise en correspondance des qualifications pour rechercher un travailleur, un candidat, ou une personne à contacter qui est qualifiée pour effectuer un type spécifique de tâche. Le recherche de recherche de mise en correspondance des qualifications entre les compétences, la formation, les certificats, les postes de confiance et le projet atteignent et soumettre les résultats qui correspondent aux critères entrés.  Par exemple, il peut être utile de connaître les travailleurs de votre organisation a acquis leur CPA.

Les profils de mise en correspondance des qualifications permettent de rechercher les employés actuels ou les candidats dont les qualifications correspondent directement aux besoins de votre entreprise.  Par exemple, vous pouvez créer un profil de mise en correspondance des qualifications pour un poste vacant de votre organisation. En créant un profil pour une tâche spécifique et en copiant les qualifications, les formations et les certificats de cette tâche dans le profil, vous pouvez rechercher rapidement des travailleurs, des candidats et des personnes à contacter qui correspondent à un ou plusieurs des critères entrés dans le profil et afficher la liste des candidats dont les qualifications correspondent le mieux aux qualifications requises pour la tâche.
