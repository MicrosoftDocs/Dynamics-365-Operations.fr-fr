...
La rémunération fixe fait référence au salaire brut normal des employés. Cette rubrique décrit les composants qui doivent être définis avant de créer un régime de rémunération fixe et d'inscrire des employés.

Les montants de rémunération fixe peuvent être calculés pour vos employés en fonction de facteurs tels que les performances, la zone et les augmentations de budget. Microsoft Dynamics 365 pour les opérations prend en charge l'étape, le niveau, et la rémunération de bande définis.
...
Vous pouvez utiliser ** des niveaux de rémunération ** pour définir la rémunération pour des tâches individuelles, pour garantir que les employés qui mettent à jour ces tâches sont payés suffisant. Sous ** des niveaux de rémunération ** la page, vous pouvez paramétrer les niveaux de rémunération requis pour chaque étape, le niveau, et plan de bande. Utilisez les boutons **Haut** et **Bas** pour mettre les niveaux dans l'ordre correct, en fonction de leur type. En définissant des niveaux de rémunération sur une tâche, vous garantissez que tous les employés occupant un poste pour cette tâche sont payés au même niveau.

### <a name="reference-points"></a>Points de référence

Les **Points de référence** sont les colonnes dans la grille qui définissent les plages de rémunération pour chaque niveau. Le niveau de rémunération est la ligne dans la grille. La référence générale points pour un plan du type de niveau sont un minimum, une valeur médiane, et un maximum. Vous créez des points de référence sur ** point de référence des paramétrages ** la page.
...
** Règle d'embauche ** le paramètre permet de spécifier si tous les employés doivent recevoir la même augmentation, indépendamment de la date qu'ils ont été engagés (** règle d'embauche ** = ** aucun **), ou si les employés doivent recevoir un pourcentage de la prime, selon le temps pendant lequel elles ont été employés au cours de le cycle (** règle d'embauche ** = ** pourcentage **). 
...
Après avoir défini les bases du régime de rémunération fixe, vous pouvez paramétrer la structure de rémunération du régime. Cliquez sur ** rémunération de paramétrage **. Un curseur de dialogue s'ouvre qui propose trois options :
...
La règle d'admissibilité prend également en compte les syndicats associés à l'employé (sous ** des employés ** page, sous ** travailleur ** l'onglet, cliquez sur ** les informations personnelles ** &gt; ** les syndicats **).
