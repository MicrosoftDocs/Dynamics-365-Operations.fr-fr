---
title: "Permet de définir et de tenir à jour les clients de canal, les registres, et les stations matérielles"
description: "Ce wiki décrit comment connecter des périphériques à votre Retail POS."
author: josaw1
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="define-and-maintain-channel-clients-registers-and-hardware-stations"></a>Permet de définir et de tenir à jour les clients de canal, les registres, et les stations matérielles

Ce wiki décrit comment connecter des périphériques à votre Retail POS.
...
Chemin d'accès : Cliquez sur ** au détail et commerce ** &gt; ** canal paramétré ** &gt; ** la configuration ** &gt; ** registres **. Le TPV est une entité utilisée pour définir les caractéristiques d'une instance spécifique du PDV. Ces informations comprennent le profil matériel ou paramètrentes pour les périphériques au détail qui seront utilisés dans le Registre, le magasin que la caisse enregistreuse est mis en correspondance, et l'expérience visuelle de l'utilisateur qui se connecte à ce registre.
...
Le PDV de cloud est un PDV navigateur-basé. Comme il fonctionne dans le navigateur, le POS de cloud ne nécessite pas Windows 8,1 (ou un système d'exploitation basé sur PC ultérieure. Si le type d’application **Retail Cloud POS** est mappé à un périphérique spécifique dans le back office, ce périphérique peut être utilisé via le navigateur sans avoir besoin de télécharger ou d’installer un package. Cloud POS nécessite une station matérielle pour utiliser le matériel au-delà de l’analyse de code-barres basée sur Wedge clavier.

### <a name="hardware-profile"></a>Profil matériel

Chemin d'accès : Cliquez sur ** commerce ** &gt; ** canal paramétré ** &gt; ** la configuration ** &gt; ** le POS profils ** &gt; ** des profils matériels **. Un profil de matériel identifie le matériel qui est connecté à une caisse enregistreuse de PDV ou une station matérielle. Le profil de matériel est également utilisé pour spécifier les paramètres de processeur de paiement qui doivent être utilisés pendant la communication avec le kit de développement de logiciel (SDK) de paiement. (Le Kit de développement logiciel de paiement est déployé dans le cadre de la station matérielle).
...
Chemin d'accès : Cliquez sur ** commerce ** &gt; ** canal paramétré ** &gt; ** la configuration ** &gt; ** le POS profils ** &gt; ** des profils de station matérielle **. Bien que la station matérielle elle-même spécifiée au niveau du canal inclut des informations spécifiques à l’instance, comme l’URL de la station matérielle, le profil de la station matérielle inclut des informations qui peuvent être statiques ou partagées entre plusieurs stations matérielles. Les informations statiques incluent le port à utiliser, le package de la station matérielle et le profil de matériel. Les informations statiques comprennent également une description du type de station matérielle en cours de déploiement, telle que **Caisse **ou **Retours**, selon le matériel qui est requis pour chaque station matérielle spécifique.

## <a name="scenarios"></a>Scénarios
### <a name="mpos-with-connected-peripheral-devices"></a>MPOS avec des périphériques connectés

[point de vente traditionnel et fixe de![(]. /media/traditional-300x279.png)](. /media/traditional.png) Pour connecter MPOS les périphériques de PDV dans un scénario traditionnel et fixe du PDV, recherchez d'abord au registre lui-même, et de leur affecter un profil matériel. Vous trouverez des registres POS ** au détail et commerce ** &gt; ** au canal paramétré ** &gt; ** la configuration ** &gt; ** des registres **. Une fois que vous avez affecté le profil de matériel, la synchronisation modifie la base de données de canal à l’aide du programme de distribution « Caisses enregistreuses ». Vous pouvez trouver les programmes de distribution ** au détail et commerce ** &gt; ** à l'IT au détail ** &gt; ** programme de distribution **. Ensuite, configurez une station matérielle « locale » sur le canal. Cliquez sur ** au détail et commerce ** &gt; ** canaux ** &gt; ** magasins de vente au détail ** &gt; ** tous les magasins de vente au détail **, puis sélectionnez un magasin. Puis, dans l'organisateur **Stations matérielles**, cliquez sur **Ajouter** pour ajouter une station matérielle. Entrez une description, entrez **localhost** comme nom d’hôte, puis synchroniser les modifications sur le canal à l’aide du programme de distribution « Configuration de canal ». Vous pouvez trouver les programmes de distribution ** au détail et commerce ** &gt; ** à l'IT au détail ** &gt; ** programme de distribution **. Enfin, dans MPOS, utilisez l'opération **Sélectionner une station matérielle** pour sélectionner la station matérielle **localhost**. Définissez la valeur de la station matérielle sur **Active**. Le profil de matériel utilisé dans ce scénario doit provenir de la caisse enregistreuse du PDV elle-même. Un profil de station matérielle n’est pas requis pour ce scénario. **Remarque :** certaines modifications apportées au profil de matériel, telles que les tiroirs-caisses, nécessitent que la nouvelle équipe ait commencé une fois que les modifications aient été synchronisées sur le canal. **Remarque :** Cloud POS doit utiliser la station matérielle autonome pour communiquer avec les périphériques de vente au détail.

### <a name="mpos-or-cloud-pos-with-a-stand-alone-hardware-station"></a>MPOS ou Cloud POS avec une station matérielle autonome

alignleft » width= "300 "d'align= " d'id= " légende\[pièce jointe\_340041 "\][périphériques partagés![] (. /media/shared-300x254.png)](. Les périphériques partagés\[/caption\] /media/shared.png) Dans ce scénario, une station matérielle autonome est partagés entre des clients de MPOS et du PDV de cloud. Ce scénario nécessite de créer un profil de station matérielle afin de spécifier le package de téléchargement, le port et le profil de matériel qu'utilise la station matérielle. Vous pouvez trouver le profil de station matérielle ** au détail et commerce ** &gt; ** au canal paramétré ** &gt; ** la configuration ** &gt; ** des profils de PDV ** &gt; ** des profils de station matérielle **. Après la création du profil de station matérielle, accédez au canal de vente au détail spécifique (** au détail et commerce ** &gt; ** des canaux ** &gt; ** les magasins de vente au détail ** &gt; ** tous les magasins de vente au détail **), puis ajoutez une nouvelle station matérielle. Mappez cette nouvelle station matérielle au profil de station matérielle créé précédemment. Ensuite, fournissez une description qui aidera le caissier à identifier la station matérielle. Dans ** nom d'hôte ** le champ, entrez l'URL d'ordinateur central au format suivant : ** https://MachineName:Port/HardwareStation**&lt;&gt;. (Remplacez **&lt;MachineName : Port&gt;** avec le nom réel machine de la station du matériel et le port spécifié dans le profil de station matérielle.) Pour une station matérielle autonome, vous devez également spécifier l'ID terminal de (EFT) de transfert électronique de fonds. Cette valeur identifie le terminal TEF connecté à la station matérielle lorsque le connecteur de paiement communique avec le fournisseur de paiement. Ensuite, à partir de la station matérielle réelle, accédez au canal et sélectionnez la station matérielle. Puis cliquez sur **Télécharger**et installez la station matérielle. Ensuite, dans MPOS ou Cloud POS, utilisez l'opération **Sélectionner une station matérielle** pour sélectionner la station matérielle qui a été précédemment installée. Sélectionnez **Jumeler** pour établir une relation sécurisée entre le PDV et la station matérielle. Cette étape doit être effectuée une fois pour chaque combinaison de POS et de station matérielle. Une fois que la station matérielle est jumelée, la même opération est utilisée pour rendre la station matérielle active alors qu’elle est utilisée. Pour ce scénario, le profil matériel doit être affectée au profil de station matérielle et non le registre. Si pour quelque raison une station matérielle n'a pas un profil matériel directement affecté, le profil matériel affecté à la caisse enregistreuse est utilisé
...
Le profil de fonctionnalité est défini au niveau du magasin. Il spécifiait les paramètres de la taille du magasin sur les fonctions qui peuvent être exécutées au niveau de le POS. Les fonctions suivantes sont gérées via le profil de la fonctionnalité. Ces fonctionnalités sont organisées par l’organisateur.
...
-   Organisateur **Codes info** :
    -   Tous les aspects de la manière dont les codes info sont gérés dans le système POS. Pour plus d'informations, voir codes info [] (information-codes-retail.md).
...
Vous pouvez créer des champs personnalisés pour ajouter les champs non prédéfinis livré au niveau de le POS. Pour plus d'informations sur l'utilisation des champs personnalisés, voir [travaillant à valider personnalisé de blog de champs] (https://blogs.msdn.microsoft.com/axsupport/2012/08/06/ax-for-retail-2012-working-with-custom-fields/).
...
-   **Désactivé** – le périphérique a été désactivé dans le back office ou par l’intermédiaire du PDV.
...
Toutes les modifications apportées à un client de PDV, à l’exception des modifications apportées à l’état d’activation du périphérique, doivent être synchronisées sur la base de données du canal pour être prises en compte. À la synchronisation devient la base de données des canaux, recherchez ** au détail et commerce ** &gt; ** à l'IT au détail ** &gt; ** programme de distribution **, puis exécutez le programme nécessité de distribution. Pour les modifications du client, vous devez exécuter les programmes de distribution « Caisses enregistreuses » et « Configuration des canaux ».
